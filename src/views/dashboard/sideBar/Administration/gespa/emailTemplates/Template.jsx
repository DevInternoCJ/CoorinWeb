...
const Template = ({
  ...
  const handleDrop = (e) => {
    ...
    if (vistaPrevia) {
      e.preventDefault();
      toast.warning(
        "No puedes agregar campos en modo Vista Previa. Desactiva la vista previa para editar."
      );
      return;
    }
    e.preventDefault();
  ...
  const handleVistaPreviaChange = (e) => {
    ...
    if (isChecked) {
      toast.info(
        "Modo Vista Previa activado. El mensaje está bloqueado para edición."
      );
    }
  };
        ...
        <div className="border-1 p-3 border-jerarquia1 rounded-lg min-h-[200px] leading-relaxed">
          ...
          >
            <input
              ...
              placeholder={
                vistaPrevia
                  ? "Vista previa activada (solo lectura)"
                  : "Mensaje (arrastra campos aquí)"
              }
            />
