    ...
    <div className="grid grid-cols-1 gap-2">
      ...
      {energia === "Si" && energiaDetalle !== "No aplica" && (
        <div className="w-full min-w-0">
          <FloatingSelect
            ...
            label="Detalle Energía"
            value={energiaDetalle}
            onChange={(e) => {
              setEnergiaDetalle(e.target.value);
              if (e.target.value === "No aplica") { setEnergia("No"); }
            }}
            options={detalleOptions.map(o => ({ value: o, label: o }))}
            disabled={disabled}
      ...
      {!(acuse === "Si" && acuseDetalle !== "No aplica") && (
        <div className="w-full">
          <FloatingSelect
            id="acuse-select-f7"
            label="¿Cuenta con acuse de cobro?"
            value={acuse}
            onChange={(e) => {
        ...
        <div className="w-full">
          <FloatingSelect
            ...
            onChange={(e) => {
              ...
              if (e.target.value === "No aplica") { setAcuse("No"); }
            }}
            options={detalleOptions.map(o => ({ value: o, label: o }))}
            disabled={disabled}
            placeholder="Selecciona alguno"
      ...
      {!(fotografia === "Si" && fotografiaDetalle !== "No aplica") && (
        <div className="w-full">
          <FloatingSelect
            id="foto-select-f7"
            label="¿Cuenta con fotografía del predio?"
            value={fotografia}
            onChange={(e) => {
