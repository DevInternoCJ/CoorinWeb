...
const CaptureVisit = ({
  ...
  useEffect(() => {
    if (mostrarTabla) {
      toast.info(
        `Capture el ${porExpediente ? "expediente" : "número de cuenta"}.`,
      );
    }
  ...
  useEffect(() => {
    if (cuentaData) {
      setCuentaData((prev) => ({
        ...prev,
        _busquedaPorExpediente: porExpediente,
      }));
    }
  }, [porExpediente]);
  ...
  useEffect(() => {
    ...
    const ilocalizable =
      domicilioData.ilocalizable || domicilioData.Ilocalizable;
    if (
      ilocalizable &&
      ilocalizable !== "" &&
      ilocalizable !== "0" &&
      ...
      setMensajeDomicilio(
        "El domicilio seleccionado cuenta con visita clasificada como 'Ilocalizable'.",
      );
      setMensajeColor("red");
    ...
    const ultimaCaptura =
      domicilioData.ultimaCaptura || domicilioData.UltimaCaptura;
    if (ultimaCaptura && ultimaCaptura !== "") {
      ...
      if (fechaUltimaCaptura === hoy) {
        setMensajeDomicilio(
          "Ya se cuenta con visita capturada el día de hoy para ese domicilio.",
        );
        setMensajeColor("red");
        setMostrarBtnCaptura(false);
        toast.error(
    ...
    const ultimaVisita =
      domicilioData.ultimaVisita || domicilioData.UltimaVisita;
    const hoy = new Date();
    const hace30Dias = new Date();
    hace30Dias.setDate(hoy.getDate() - 30);
  ...
  const handleBuscarCuenta = async () => {
    ...
    try {
      const cuentaOrExpedienteStr = String(
        porExpediente ? idCuenta : parseInt(idCuenta, 10),
      );
      let res;
      try {
        ...
        expandir = false;
        toast.error(
          "La cuenta a la que desean acceder no existe, verifíquela por favor",
        );
      ...
      if (res && res.status === 200 && res.data && res.data.cuenta) {
        ...
        if (porExpediente) {
          ...
          setExpedienteLabel(
            cuentaInfo.expediente || cuentaInfo.Expediente || "",
          );
        }
        setNombreDeudor(
          cuentaInfo.nombreDeudor || cuentaInfo.NombreDeudor || "",
        );
        ...
        if (domicilios.length > 0) {
          ...
          const idProducto =
            primerDomicilio.idProducto || primerDomicilio.IdProducto;
          const mostrarEnergia =
            idProducto === 52 || idProducto === 22 || idProducto === 1;
  ...
  const validarCaptura = () => {
    ...
    const contactoEsTitular =
      dataF4.idContacto && dataF4.valorContacto === "Titular";
    const contactoEsLeConoce =
      dataF4.idContacto && dataF4.valorContacto === "Le conoce";
    ...
    if (dataF4.horaVisita) {
      ...
      if (hora < 7 || hora > 21) {
        toast.error(
          "La hora de la visita debe de ser dentro del horario laboral (7hrs - 22hrs).",
        );
        return false;
      }
    } else {
    ...
    if (carteraNumero === 14 || carteraNumero === 24) {
      ...
      if (
        dataF7.acuseRequerimiento == null ||
        dataF7.acuseRequerimiento === ""
      ) {
        toast.error(
          "Seleccione si el deudor cuenta con acuse de requerimiento de cobro.",
        );
        return false;
      }
    ...
    let callesVacias = [];
    if (!dataF5.calleHorizontalNorte?.trim())
      callesVacias.push("Horizontal norte");
  ...
  const ejecutarCaptura = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        fechaPagoNegociacion: dataF4.montoNegociacion
          ? dataF4.fechaPagoNegociacion + "T00:00:00"
          : null,
        telefonosCapturados: dataF6.telefonosCapturados || [],
        numeroMedidor: dataF7.numeroMedidor || null,
        energiaElectrica:
          dataF7.energiaElectrica != null
            ? dataF7.energiaElectrica === "Si" ||
              dataF7.energiaElectrica === true
            : null,
        acuseRequerimiento:
          dataF7.acuseRequerimiento != null
            ? dataF7.acuseRequerimiento === "Si" ||
              dataF7.acuseRequerimiento === true
            : null,
        fotografiaPredio:
          dataF7.fotografiaPredio != null
            ? dataF7.fotografiaPredio === "Si" ||
              dataF7.fotografiaPredio === true
            : null,
      ...
      if (response && response.status === 200) {
        ...
        if (
          (carteraNumero === 14 || carteraNumero === 24) &&
          coordenadasNoEncontradas
        ) {
          toast.warning(
            "Visita guardada exitosamente. No se encontraron datos (latitud y/o longitud) del domicilio seleccionado.",
            {
              duration: 6000,
            },
          );
      ...
      {mostrarTabla ? (
        <>
          ...
          <div className="w-full px-[1vw] pt-[0.5vw] pb-2 flex items-center gap-4">
            <h2 className="text-lg font-semibold text-jerarquia3 truncate">
              {tipoInformacion}
            </h2>
          </div>
          <div className="flex flex-col w-full sm:grid sm:grid-cols-12 sm:items-center sm:gap-0 px-[1vw]">
            ...
            <div className="relative w-full min-w-0 flex flex-col items-center mb-2 sm:col-span-1 sm:w-[6.66vw] sm:mb-0">
              ...
              <label
                htmlFor="cartera-select-carga-visits"
            ...
            <div className="w-full flex flex-col items-center justify-center mb-2 sm:col-span-1 sm:w-[6vw] sm:ml-[1vw] sm:mb-0">
              <label
                htmlFor="porExpediente"
                className="text-sm font-medium mb-[0.25rem] text-center flex items-center justify-center h-full"
              >
                Expte.
              </label>
              <input
            ...
            <div className="relative w-full max-w-lg flex flex-col items-center justify-center mb-2 sm:col-span-6 sm:w-[28vw] sm:ml-[-1vw] sm:mb-0">
              <select
                title={
                  !tienePermisoCaptura
                    ? "Sin permisos para capturar visitas"
                    : "Selecciona alguno"
                }
              ...
              >
                <option value="" hidden></option>
                {cuentaData &&
                  Array.isArray(cuentaData.domicilios) &&
                  cuentaData.domicilios.length > 0 &&
                  cuentaData.domicilios.map((dom, idx) => (
                    <option key={idx} value={dom.domicilio}>
                      {dom.domicilio}
                    </option>
                  ))}
              </select>
          ...
          <div
            ...
            onClick={(e) => {
              if (!direccionSeleccionada) {
                e.stopPropagation();
                toast.warning(
                  "Primero selecciona un domicilio antes de continuar",
                );
              }
            }}
          >
            <div className="flex flex-col gap-4 sm:flex-row sm:gap-4 mb-4 w-full">
              <div className="w-full sm:flex-1 min-w-0">
                <CapturaVisitsF2
                  ...
                  filtroExcluirId={filtroHabitacion}
                />
              </div>
              <div className="w-full sm:flex-[2] min-w-0">
                <CapturaVisitsF4
                  ref={f4Ref}
                  key={`f4-${resetKey}`}
                  mapeoVivienda={mapeoVivienda}
                />
              </div>
            </div>
            <div className="flex flex-col gap-4 sm:flex-row sm:gap-4 w-full">
              <div className="w-full sm:flex-1 min-w-0">
                <CapturaVisitsF3 ref={f3Ref} key={`f3-${resetKey}`} />
              </div>
              <div className="w-full sm:basis-1/4 min-w-0">
                <CapturaVisitsF5
                  ref={f5Ref}
                  key={`f5-${resetKey}`}
              ...
              {mostrarF6 && (
                <div className="w-full sm:basis-1/6 min-w-0">
                  <CapturaVisitsF6 ref={f6Ref} key={`f6-${resetKey}`} />
                </div>
              )}
              {mostrarF7 && (
                <div className="w-full sm:basis-1/4 min-w-0">
          ...
          <div className="w-full mt-4 flex justify-center">
            <button
              ...
              disabled={
                !direccionSeleccionada || !mostrarBtnCaptura || guardando
              }
        ...
        <>
          ...
          <div className="flex flex-row items-center gap-6 w-full px-6 py-4">
            ...
            <div className="flex flex-col flex-1 justify-center">
              <h2 className="text-lg font-semibold text-jerarquia3 truncate mb-2">
                {tipoInformacion}
              </h2>
              <div className="flex flex-row w-full gap-4">
                ...
                <div className="flex flex-col flex-1 items-center justify-start">
                  <label
                    htmlFor="porExpediente"
                    className="text-sm font-medium mb-1 text-center flex items-center justify-center h-full"
                  >
                    Expdte.
                  </label>
                  <input
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 p-6">
            ...
            <p className="text-gray-600 mb-2">
              Las siguientes calles están vacías:
            </p>
            <p className="text-gray-800 font-medium mb-4 pl-4">
              {callesVaciasTexto}
            </p>
