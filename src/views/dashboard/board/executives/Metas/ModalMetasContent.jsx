...
const ModalMetasContent = () => {
  ...
  const validateAndSetField = (
    ...
    if (
      (field === "negociaciones" || field === "titulares") &&
      eventType === "change"
    ) {
      const negociacionesNum =
        Number(field === "negociaciones" ? value : allValues.negociaciones) ||
        0;
      const titularesNum =
        Number(field === "titulares" ? value : allValues.titulares) || 0;
      let errors = [];
      if (
        negociacionesNum > 0 &&
        titularesNum > 0 &&
        negociacionesNum >= titularesNum
      ) {
        errors.push({
          msg: "Las negociaciones deben ser menor a los contactos con titulares",
        });
      }
      ...
      if (
        cumplimientosNum > 0 &&
        negociacionesNum > 0 &&
        cumplimientosNum > negociacionesNum
      ) {
        errors.push({
          msg: "Las negociaciones deben ser mayor o igual a los cumplimientos",
        });
      }
      setValidationState((prev) => ({
        ...prev,
        negociaciones: {
          ...prev.negociaciones,
          errors,
          showIcon: errors.length > 0,
        },
        titulares: {
          ...prev.titulares,
          error: errors.find((e) => e.msg.includes("titulares"))
            ? "Los contactos con titulares deben ser mayor a las negociaciones"
            : false,
          showIcon: errors.find((e) => e.msg.includes("titulares"))
            ? true
            : false,
        },
      }));
    ...
    if (field === "titulares" || field === "cuentas") {
      ...
      const titularesValue =
        field === "titulares" ? value : allValues.titulares;
      ...
      let errorTitulares = false,
        showIconTitulares = false,
        requiredErrorTitulares = false;
      let errorCuentas = false,
        showIconCuentas = false,
        requiredErrorCuentas = false;
    ...
    if (field === "cumplimientos" || field === "negociaciones") {
      const cumplimientosNum =
        Number(field === "cumplimientos" ? value : allValues.cumplimientos) ||
        0;
      const negociacionesNum =
        Number(field === "negociaciones" ? value : allValues.negociaciones) ||
        0;
      if (
        ...
        setValidationState((prev) => ({
          ...
          negociaciones: {
            ...prev.negociaciones,
            error:
              "Las negociaciones deben ser mayor o igual a los cumplimientos",
            showIcon: true,
          },
        }));
        if (showToast)
    ...
    switch (field) {
      ...
      case "negociaciones": {
        ...
        updateValidation("negociaciones", {
          errors,
          showIcon: showIconNeg,
          requiredError: reqError,
        });
        ...
        if (
          allValues.cumplimientos !== undefined &&
      ...
      case "cumplimientos": {
        ...
        if (
          allValues.negociaciones !== undefined &&
      ...
      case "montoCumplido": {
        if (value === "" || value === null) {
          ...
          if (!validateCurrencyInput(value)) {
            ...
            const saldo =
              Number(parseCurrencyToNumber(allValues.saldoSolucionado)) || 0;
        ...
        console.log("montoCumplido validation:", {
          error,
          showIcon,
          requiredError,
        });
        ...
        if (
          allValues.saldoSolucionado !== undefined &&
      ...
      case "saldoSolucionado": {
        if (value === "" || value === null || value === undefined) {
          ...
          if (!validateCurrencyInput(value)) {
            ...
            const monto =
              Number(parseCurrencyToNumber(allValues.montoCumplido)) || 0;
        ...
        console.log("saldoSolucionado validation:", {
          error,
          showIcon,
          requiredError,
        });
        updateValidation("saldoSolucionado", {
          error,
          showIcon,
          requiredError,
        });
        ...
        if (
          allValues.montoCumplido !== undefined &&
      ...
      case "horaEntrada": {
        if (value === "" || value === null) {
          ...
          if (!timeRegex.test(String(value))) {
            ...
            if (hh < 7 || hh > 15) {
              error =
                "El horario de entrada debe de ser entre las 7 y 15 horas.";
              showIcon = true;
            }
          }
        }

        console.log("horaEntrada validation:", {
          error,
          showIcon,
          requiredError,
        });
      ...
      case "horaSalida": {
        if (value === "" || value === null) {
          ...
          if (!timeRegex.test(String(value))) {
            ...
            if (hhSalida < 7 || hhSalida > 22) {
              error =
                "El horario de salida debe ser entre las 07:00 y 22:00 horas.";
              showIcon = true;
            } else if (
              ...
              const [hhEntrada, mmEntrada] = allValues.horaEntrada
                .split(":")
                .map(Number);
              let diff =
                hhSalida * 60 +
                (mmSalida || 0) -
                (hhEntrada * 60 + (mmEntrada || 0));
              if (diff >= 600) {
                // 600 minutos = 10 horas
                error =
                  "Debe haber menos de 10 horas entre la entrada y la salida.";
                showIcon = true;
              }
            }
          }
        ...
        console.log("horaSalida validation:", {
          error,
          showIcon,
          requiredError,
        });
    ...
    if (
      ...
      if (titularesNum > 0 && cuentasNum > 0 && titularesNum >= cuentasNum) {
        if (field === "titulares") {
          updateValidation("titulares", {
            error: "Titulares debe ser menor que Cuentas",
            showIcon: true,
            requiredError: requiredErrorTitulares,
          });
        }
        if (field === "cuentas") {
          updateValidation("cuentas", {
            error: "Cuentas debe ser mayor que Titulares",
            showIcon: true,
            requiredError: requiredErrorCuentas,
          });
        }
        if (showToast) {
  ...
  useEffect(() => {
    ...
    const idEjecutivoSesionLocal =
      userDataForIds?.idEjecutivo ||
      userDataForIds?.idejecutivo ||
      userDataForIds?.id ||
      null;
    ...
    if (
      ...
      Number(selectedExecutiveNode) === Number(idEjecutivoSesionLocal) &&
      Array.isArray(allHierarchyIds) &&
      allHierarchyIds.length > 0
    ) {
      idsToSend = allHierarchyIds
  ...
  const handleGuardar = async () => {
    ...
    const montoCumplido =
      Number(parseCurrencyToNumber(inputValues.montoCumplido)) || 0;
    const saldoSolucionado =
      Number(parseCurrencyToNumber(inputValues.saldoSolucionado)) || 0;
        ...
        for (const idEjecutivo of selectedExecutives) {
          ...
          const payload = {
            ...
            horaEntrada: inputValues.horaEntrada
              ? inputValues.horaEntrada.length === 5
                ? inputValues.horaEntrada + ":00"
                : inputValues.horaEntrada
              : "",
            horaSalida: inputValues.horaSalida
              ? inputValues.horaSalida.length === 5
                ? inputValues.horaSalida + ":00"
          ...
          <div className="bg-jerarquia1/10 rounded-lg p-2 sm:p-3 shadow border border-jerarquia1/20 w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2 items-start">
              ...
              <div className="w-full flex items-end">
                {(() => {
                  const montoCumplido =
                    Number(parseCurrencyToNumber(inputValues.montoCumplido)) ||
                    0;
                  const saldoSolucionado =
                    Number(
                  ...
                  const hasMoneyValidationError =
                    montoCumplido > 0 &&
                    saldoSolucionado > 0 &&
                    montoCumplido >= saldoSolucionado;
                  const hasTinyIntValidationError =
                    negociaciones > 255 || cumplimientos > 255;
                  const hasRelationValidationError =
                    cumplimientos > 0 &&
                    negociaciones > 0 &&
                    cumplimientos > negociaciones;
                  const hasValidationError =
                    hasMoneyValidationError ||
                    hasTinyIntValidationError ||
                    hasRelationValidationError;
                  const isDisabled =
                    selectedRows.length === 0 || hasValidationError;
