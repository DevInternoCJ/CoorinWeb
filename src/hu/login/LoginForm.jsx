...
const LoginForm = ({ onLoginSuccess }) => {
  ...
  const handleInputChange = useCallback((field, value) => {
    if (field === 'username') {
      const formattedValue = value.replace(/[^a-zA-Z]/g, '').toUpperCase().slice(0, 4);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        processSuccessfulLogin(response, passwordValidation, onLoginSuccess, navigate);   
      } catch (validationError) {
        handlePasswordValidationError(validationError, onLoginSuccess);
      }
    } catch (error) {
      console.error('Error en inicio de sesión:', error);
      const errorMessage = error.response?.data?.loginResult?.Mensaje || ERROR_MESSAGES.LOGIN_ERROR;
      setApiError(errorMessage);
      toast.error(errorMessage);
    } finally {
      setLoading(false);
  ...
  return (
    <form onSubmit={handleSubmit} className="lg:w-full px-6 text-neutral-900">
      ...
      {apiError && (
        <p className="text-red-500 text-sm mt-1 mb-2">{apiError}</p>
      )}   
      <ButtonLogin
        type="submit"
        loading={loading}
