...
const MODAL_SIZES = {
  ...
  consultaVisits: {
    maxWidth: "min(644px, 95vw)",
    minWidth: "320px",
    width: "min(483px, 95vw)",
    height: "506px",
    maxHeight: "90vh",
  },
  capturaVisit: {
...
const ModalBaseInformacion = ({
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      toast.dismiss(loadingToast);
      toast.error(
        "Error al exportar los datos, verifique la conexcion a internet."
      );
  ...
  const isOffersTab =
    isInformacion &&
    carouselNav &&
    tabsList[carouselNav.currentIndex]?.key === "Ofrecimientos";
  ...
  if (isConsultaVisitas) normalizedSize = "consultaVisits";
    ...
    const isPagosReportadosTab =
      carouselNav &&
      tabsList[carouselNav.currentIndex]?.key === "Pagos Reportados";
  ...
  if (carouselNav) {
    console.log(
      ...
      tabsList[carouselNav.currentIndex]?.key
    );
  ...
  return (
    <div className="modal-blur-bg">
      ...
      <div
        ref={modalRef}
        className={`modal-content bg-white rounded-lg shadow-2xl overflow-hidden flex flex-col w-full mx-auto my-8 max-h-[90vh] h-auto ${modalClassName} ${
                      ...
                      return (
                        ...
                        >
                          ...
                          {/*
                                                    ...
                                                    - lg, xl, 2xl: icono + texto
                                                */}
                          <span className="hidden md:hidden lg:inline xl:inline 2xl:inline">
                            {shortText}
                          </span>
                        </button>
                      );
                ...
                {infoCuenta && infoCuenta.cuenta && (
                  ...
                  >
                    {infoCuenta._busquedaPorExpediente ? (
                      <>
                        <span
                          className="font-semibold text-base text-jerarquia3"
                          style={{ marginRight: "3.125rem" }}
                        >
                          {infoCuenta.cuenta.idCuenta}
                        </span>
                        <span className="font-semibold text-base text-jerarquia3">
                          {infoCuenta.cuenta.nombreDeudor}
                        </span>
                      </>
                    ) : (
                      <>
        ...
        <div className={`flex-1 w-full overflow-auto ${contentClassName}`}>
          {isInformacion ? (
            <>
              ...
              {isOffersTab && (
                <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 2xl:grid-cols-5 gap-3 px-4 pt-4 pb-2 w-full">
                  {/* Cartera */}
                  ...
                  <div className="relative flex-1 min-w-0 max-w-xs">
                    ...
                    {loadingConsultas && (
                      <span className="text-[8px] text-gray-500 absolute right-8 top-1">
                        Cargando...
                      </span>
                    )}
                    {errorConsultas && (
                      <span className="text-[8px] text-red-500 absolute right-8 top-1">
                        {errorConsultas}
                      </span>
                    )}
                  </div>
