...
const DarkListContent = () => {
    ...
    const handleBuscar = async (e) => {
        ...
        if (tipo === "correo" && !/^.+@.+\.[a-zA-Z]{2,}$/.test(valor)) {
            toast.error("El correo debe tener el formato usuario@dominio.com para poder buscar.");
            return;
        }
        if (tipo === "telefono") {
        ...
        if (tipo === "correo") {
            ...
            if (/^[_\-.,:;.]/.test(local) || /[_\-.,:;.]$/.test(local)) {
                toast.error("El correo no debe iniciar ni terminar con símbolos antes del @.");
                return;
            }
            ...
            if (!/\.[a-zA-Z]{2,}$/.test(domain)) {
                toast.error("El dominio debe terminar con un punto y al menos dos letras. Ejemplo: usuario@dominio.com");
                return;
            }
            if (!/^[a-zA-Z0-9][a-zA-Z0-9._-]*[a-zA-Z0-9]@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/.test(valor)) {
                toast.error("El correo solo puede contener letras, números, puntos, guiones y guiones bajos antes del @. Ejemplo: usuario@dominio.com");
                return;
            }
        }
        setLoading(true);
