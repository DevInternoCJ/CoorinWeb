...
import { topCampaign } from "../../../../services/LokiServices";
...
const ModalToponeHundred = ({ open, onClose, idCampaña }) => {
        ...
        if (open && idCampaña) {
            ...
            topCampaign({ idCampaña })
                ...
                .catch(err => {
                    ...
                    if (err?.response?.status === 404 && typeof err?.response?.data === 'string' && err.response.data.includes('No se encontraron datos')) {
                        setRows([]);
    ...
    return (
        <div style={{
            ...
            <div style={{ background: 'white', borderRadius: 8, padding: 24, minWidth: 420, boxShadow: '0 2px 16px rgba(0,0,0,0.15)', position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start' }}>
                {/* Botón de cerrar en la esquina superior derecha */}
                <button
                    className="modal-btn modal-btn-close"
                    style={{ fontSize: 20, position: 'absolute', right: 12, top: 8, zIndex: 10 }}
                    onClick={onClose}
                    aria-label="Cerrar"
                >
                    &times;
