...
import { newCampaign } from "../../../../services/LokiServices";
...
const NewCampaign = ({ onCreated }) => {
    ...
    const handleNuevaCampania = async () => {
        ...
        try {
            ...
            const idEjecutivoInsert = userData?.idEjecutivo || userData?.idejecutivo || userData?.id || null;
    ...
    return (
        <div style={{ display: 'flex', gap: '0.5rem', marginTop: 8, justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
            ...
            {errorNueva && (
                <div style={{ color: 'red', textAlign: 'center', marginTop: 4, fontSize: 13 }}>{errorNueva}</div>
            )}
        </div>
    );
};
