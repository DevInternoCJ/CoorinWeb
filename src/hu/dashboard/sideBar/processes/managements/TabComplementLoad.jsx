...
const TabComplementLoad = () => {
  ...
  const validateFieldValue = (campo, valor, rowIndex) => {
    ...
    switch (campo) {
      ...
      case "HoraGestion": {
        ...
        const is12Hour = /^(0?[1-9]|1[0-2]):[0-5][0-9]\s?(am|pm|AM|PM)$/.test(
          val,
        );
      ...
      case "Comentario":
        ...
        if (val.length > 8000) {
          errors.push(
            `Fila ${rowIndex + 2}: ${campo} no debe exceder 8000 caracteres (tiene ${val.length})`,
          );
        }
        break;
      ...
      case "Extension":
        ...
        if (!/^\d{4}$/.test(extension)) {
          errors.push(
            `Fila ${rowIndex + 2}: ${campo} debe tener exactamente 4 dígitos (ejemplo: 0000, 1234)`,
          );
        }
        break;
      ...
      case "Duracion":
        ...
        if (!/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$/.test(val)) {
          errors.push(
            `Fila ${rowIndex + 2}: ${campo} debe tener formato hh:mm:ss (ejemplo: 00:05:15)`,
          );
        }
        break;
  ...
  const processFileData = (data, headers) => {
    ...
    if (headers.length !== 10) {
      ...
      setValidationMessage(
        `El archivo tiene ${headers.length} columnas, pero se esperan exactamente 10 columnas.`,
      );
      toast.error(
        `El archivo tiene ${headers.length} columnas, pero se esperan exactamente 10 columnas.`,
        { duration: 8000 },
    ...
    if (dataErrors.length > 0) {
      ...
      setValidationMessage(
        `Se encontraron ${dataErrors.length} errores de validación.`,
      );
    } else {
  ...
  const handleCargar = async () => {
    ...
    try {
      setIsLoading(true);
      console.log(
        "[Archivo] Iniciando carga de gestiones complemento. Archivo:",
        archivo,
      );
      toast.loading("Cargando gestiones complemento...", {
        id: "cargar-loading",
      });
      ...
      if (result.data instanceof ArrayBuffer) {
        try {
          ...
          if (json.success) {
            ...
            if (json.errores && json.errores.length > 0) {
              ...
              toast.warning(
                `Se encontraron ${json.errores.length} errores. Revise la consola para más detalles.`,
              );
            }
            ...
            // Error en la carga
            const errorMsg =
              json.message || "Error al cargar las gestiones complemento.";
            ...
            if (json.errores && json.errores.length > 0) {
              ...
              if (json.errores[0]) {
                toast.warning(`Detalle: ${json.errores[0]}`, {
                  duration: 6000,
                });
              }
            }
        ...
        if (result.data.success) {
          const mensaje =
            ` ${result.data.message || "Carga completada"}\n` +
          ...
          const errorMsg =
            result.data.message || "Error al cargar las gestiones complemento.";
          toast.error(errorMsg, { duration: 6000 });
          ...
          if (result.data.errores && result.data.errores.length > 0) {
            ...
            if (result.data.errores[0]) {
              toast.warning(`Detalle: ${result.data.errores[0]}`, {
                duration: 6000,
              });
            }
          }
