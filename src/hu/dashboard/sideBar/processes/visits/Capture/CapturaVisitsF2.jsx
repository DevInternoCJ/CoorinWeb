...
const CapturaVisitsF2 = forwardRef(({ onMapeoChange, filtroExcluirId }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    getData: () => ({
      idHabitacion: mapeoSeleccionado ? parseInt(mapeoSeleccionado, 10) : null,
  ...
  useEffect(() => {
    getCatalogoValueCard()
      .then(data => {
        ...
        const mapeoOptions = data.filter(item => 
          item.idCatálogo === ID_CATALOGO_HABITACION && item.idValor !== 2807
        );
        setOpcionesMapeo(mapeoOptions);
        
        // Catálogo 25 - Viviendas
        const viviendaOptions = data.filter(item => item.idCatálogo === ID_CATALOGO_VIVIENDAS);
        setOpcionesVivienda(viviendaOptions);
        
        // Catálogo 27 - Económicos
        const economicoOptions = data.filter(item => item.idCatálogo === ID_CATALOGO_ECONOMICOS);
  ...
  const getSelectClass = (enabled) => `peer p-4 pe-9 block w-full border-transparent rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-jerarquia1 focus:border-jerarquia1 focus:pt-6 focus:pb-2 not-placeholder-shown:pt-6 not-placeholder-shown:pb-2 autofill:pt-6 autofill:pb-2 ${
