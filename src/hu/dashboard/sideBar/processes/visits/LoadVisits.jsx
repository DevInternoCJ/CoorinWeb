...
const LoadVisitsContent = ({ mostrarTabla, setMostrarTabla, onClose }) => {
    ...
    const camposEsperadosExcel = [
        'Cuenta', 'MapeoVivienda', 'ColorFachada', 'ColorPuerta', 'ColorHerreria', 
        'NivelesPisos', 'TipoVivienda', 'NivelEconomico', 'PropietarioVivienda', 
        'AutoMapeo', 'AutoMarca', 'AutoModelo', 'AutoAño', 'AutoPlacas', 
        'PersonaAtendio', 'Contacto', 'Parentesco', 'Situacion', 'CausaNoPago', 
        'FechaVisita', 'HoraVisita', 'ClaveVisitador', 'Observación', 
        'Teléfono1Visita', 'Teléfono2Visita', 'Teléfono3Visita', 'Correo', 
        'CalleHorizontalNorte', 'CalleHorizontalSur', 'CalleVerticalEste', 
        'CalleVerticalOeste', 'Sucursal', 'PaqueteVisitas', 'MontoNegociación', 
        'FechaPagoNegociación', 'Herramienta', 'NúmeroMedidor', 'EnergíaElectrica', 
        'AcuseRequerimiento', 'FotografíaPredio', 'Latitud', 'Longitud'
    ];
    ...
    const validateFieldValue = (campo, valor, rowIndex) => {
        ...
        // Campos obligatorios
        const obligatorios = ['Cuenta', 'Contacto', 'FechaVisita', 'HoraVisita', 'ClaveVisitador', 'Observación', 'Sucursal'];
        ...
        switch (campo) {
            ...
            case 'AutoMarca':
                ...
                if (!/^[a-záéíóúüñA-ZÁÉÍÓÚÜÑ\s]+$/.test(val)) {
                    errors.push(`Fila ${rowIndex + 2}: ${campo} debe contener solo letras`);
                }
                break;
            ...
            case 'AutoModelo':
                ...
                if (!/^[a-záéíóúüñA-ZÁÉÍÓÚÜÑ0-9\s]+$/.test(val)) {
                    errors.push(`Fila ${rowIndex + 2}: ${campo} debe contener solo letras y números`);
                }
                break;

            case 'AutoAño':
                if (!/^\d{4}$/.test(val)) {
                    errors.push(`Fila ${rowIndex + 2}: ${campo} debe tener exactamente 4 dígitos`);
                }
                break;

            case 'AutoPlacas':
                if (val.length > 10) {
                    errors.push(`Fila ${rowIndex + 2}: ${campo} no debe exceder 10 caracteres`);
                }
            ...
            case 'HoraVisita':
                if (!/^(0?[1-9]|1[0-2]):[0-5][0-9]\s?(am|pm|AM|PM)$/.test(val) && !/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/.test(val)) {
            ...
            case 'Observación':
                if (val.length > 8000) {
                    errors.push(`Fila ${rowIndex + 2}: ${campo} no debe exceder 8000 caracteres`);
                }
                break;
            ...
            case 'PaqueteVisitas':
                if (!/^\d+$/.test(val)) {
                    errors.push(`Fila ${rowIndex + 2}: ${campo} debe ser un número entero`);
                }
                break;

            case 'MontoNegociación':
                if (!/^\d+(\.\d{2})?$/.test(val)) {
                    errors.push(`Fila ${rowIndex + 2}: ${campo} debe ser un número con dos decimales (ej: 12556.00)`);
                }
                break;

            case 'NúmeroMedidor':
                if (val.length > 15) {
                    errors.push(`Fila ${rowIndex + 2}: ${campo} no debe exceder 15 caracteres`);
                }
                if (!/^[a-zA-Z0-9]+$/.test(val)) {
                    errors.push(`Fila ${rowIndex + 2}: ${campo} debe contener solo letras y números`);
                }
    ...
    const processFileData = (data, headers) => {
        ...
        if (headers.length !== 42) {
            ...
            toast.error(`El archivo tiene ${headers.length} columnas, pero se esperan exactamente 42 columnas.`, { duration: 8000 });
                ...
                if (errorData instanceof ArrayBuffer) {
                    try {
                        ...
                        try {
                            ...
                            errorMessage = jsonError.message || jsonError.error || decodedText;
                        } catch {
    ...
    <>
        ...
        <div className="flex flex-col items-center mx-auto px-2 sm:px-4" style={{ width: '100%', height: '100%' }}>
            ...
            <div className="col-span-1 sm:col-span-3 text-center sm:text-left">
                <span className="text-lg font-semibold text-jerarquia3">Carga Visitas - Coorin</span>
            </div>
                        ...
                        <thead>
                            <tr>
                                {camposEsperadosExcel.map((campo, index) => (
                                    <th key={index} style={{ whiteSpace: 'nowrap', position: 'sticky', top: 0, zIndex: 10 }}>
                                        {campo}
                                    </th>
                                ))}
                            </tr>
                        ...
                        <tbody>
                            {fileData.length === 0 ? (
                                <tr style={{ height: '210px' }}>
                                    <td colSpan={camposEsperadosExcel.length} style={{ textAlign: 'center', verticalAlign: 'middle', padding: '8px' }}>
                                        <span className="text-gray-600 text-sm">Seleccione un archivo para subir visitas.</span>
                                    </td>
                                </tr>
                            ) : (
                                fileData.map((row, index) => (
                                    <tr key={index} style={{ borderBottom: '1px solid #e5e7eb' }}>
                                        {camposEsperadosExcel.map((campo, idx) => (
                                            <td key={idx} style={{ padding: '8px', textAlign: 'center', whiteSpace: 'nowrap' }}>
                                                {row[campo] || ''}
                                            </td>
                                        ))}
                                    </tr>
