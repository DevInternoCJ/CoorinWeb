...
import { getCatalogoCard, getCatalogoValueCard } from "../../../../services/LokiServices";
...
const ModalCatalogosContent = () => {
        ...
        const loadCatalogValues = async () => {
            try {
                ...
                if (response && Array.isArray(response)) {
                    ...
                    console.warn('⚠️ Respuesta vacía o en formato incorrecto para valores:', response);
    ...
    const getSortedValuesData = () => {
        ...
        if (selectedCatalog) {
            ...
            const selectedCatalogData = catalogsData.find(cat => cat.catalog === selectedCatalog);
            
            if (selectedCatalogData) {
                // Filtrar los valores que corresponden al idCatalogo seleccionado
                valuesData = catalogsValuesData.filter(value => value.idCatálogo === selectedCatalogData.idCatalogo);
                
                console.log('🔍 Valores filtrados para idCatalogo', selectedCatalogData.idCatalogo, ':', valuesData.length);
            }
        }
    ...
    return (
        <div style={{
            ...
            <div className="bg-white rounded-lg p-4 shadow border border-[var(--color-jerarquia1)] flex flex-col" style={{ flex: 1, minHeight: "280px", maxHeight: "280px" }}>
                <div className="flex items-center mb-3 w-full">
                    <span className="text-sm font-semibold pl-1" style={{ color: "var(--color-jerarquia2)", minWidth: 100 }}></span>
                </div>
                <div className="scrollbar-gray" style={{ overflowX: "auto", overflowY: "auto", maxHeight: "200px", height: "100%", flex: 1, minHeight: "200px" }}>
                    <table className="modal-table">
                        <thead>
                            <tr>
                                <th style={{ width: "33%", cursor: "pointer", userSelect: "none" }} 
                                    onClick={() => handleCatalogSort('catalog')}
                                    title="Haz clic para ordenar">
                                    Catálogo{getSortIcon('catalog', catalogSort)}
                                </th>
                                <th style={{ width: "67%", cursor: "pointer", userSelect: "none" }} 
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                getSortedCatalogsData().map((item) => (
                                    ...
                                    >
                                        <td className={`font-semibold ${selectedCatalog === item.catalog ? 'text-blue-600' : ''}`}>
                                            {item.catalog}
                                        </td>
                                        <td>
                                            {item.description}
                            ...
                            {loading || loadingValues ? (
                                ...
                                getSortedValuesData().map((item) => (
                                    ...
                                    >
                                        ...
                                        <td>
                                            {item.detalle || 'Sin descripción disponible'}
                                        </td>
                                    </tr>
                                ))
                            )}
