...
const EncargadosModal = ({
    ...
    Promise.all([
      ...
      .then(([carterasData, carterasProductosData, encargadosData]) => {
        ...
        const productosDelEndpoint = carterasProductosData
          .filter((item) => item.cartera === (carterasData[0]?.cartera || ""))
  ...
  const filtrarEncargados = useCallback(
    (carteraSeleccionada, productoSeleccionado, encargadosOriginales) => {
      if (!carterasProductosData.length || !encargadosOriginales.length)
        return [];
      let filtrados = [];
  ...
  const [contadorEncargados, setContadorEncargados] = useState({
    asignados: 0,
    total: 0,
  });
  const [nodoEjecutivoHeader, setNodoEjecutivoHeader] = useState("");
  const [handleCambiarAsignacionContent, setHandleCambiarAsignacionContent] =
    useState(null);
  ...
  const cambiarButton = (
    <button
      ...
      className={`btn-success w-full sm:w-auto sm:min-w-[120px] px-4 py-2 text-base font-medium rounded-lg shadow-sm flex justify-center${
        isChangingAssignment || loading ? " opacity-70 cursor-not-allowed" : ""
      }`}
