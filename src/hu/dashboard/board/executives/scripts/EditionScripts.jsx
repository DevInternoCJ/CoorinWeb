...
const EditionScripts = ({
  ...
  useEffect(() => {
    if (!selectedScript && scripts.length > 0) {
      const defaultScript = scripts.find((s) => s.idScript === 0) || scripts[0];
  ...
  const insertFormatMarker = (marker) => {
    ...
    if (!selection.rangeCount)
      return toast.error("Selecciona un texto para aplicar el formato");
  ...
  const handleSave = async () => {
    if (!editedData.nombre)
      return toast.error("El nombre del script es obligatorio");
    if (!selectedScript)
      return toast.error("Selecciona un script antes de guardar");
    const scriptData = {
