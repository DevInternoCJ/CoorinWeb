  ...
  useEffect(() => {
    ...
    obetenerDropdownsEncargados()
      .then((data) => {
        ...
        const uniqueCarteras = Array.from(
          new Set(data.map((item) => item.idCartera))
        );
        ...
        const productosFiltrados = Array.from(
          new Set(
            data
              .filter((item) => item.idCartera === (uniqueCarteras[0] || ""))
        ...
        setSelectedEncargado(
          data.find(
            (item) =>
              item.idCartera === (uniqueCarteras[0] || "") &&
              item.idProducto === (productosFiltrados[0] || "")
          )?.idEjecutivo || null
        );
      })
      .catch((err) => {
  ...
  useEffect(() => {
    const fetchExecutiveTree = async () => {
      ...
      try {
        ...
        toast.error(
          "Error al obtener la jerarquía de ejecutivos. Inténtalo de nuevo.",
          e
        );
            ...
            <select
              ...
              onChange={(e) => {
                ...
                const productosFiltrados = Array.from(
                  new Set(
                    encargados
                      .filter((item) => item.idCartera === e.target.value)
                      .map((item) => item.idProducto)
                  )
                );
                setProductos(productosFiltrados);
                ...
                const encargadosFiltrados = encargados.filter(
                  (item) =>
                    item.idCartera === e.target.value &&
                    item.idProducto === (productosFiltrados[0] || "")
                );
                setSelectedEncargado(
                  encargadosFiltrados[0]?.idEjecutivo || null
                );
              }}
          ...
          <div style={{ position: "relative" }}>
            <select
              ...
              onChange={(e) => {
                ...
                const encargadosFiltrados = encargados.filter(
                  (item) =>
                    item.idCartera === cartera &&
                    item.idProducto === e.target.value
                );
                setSelectedEncargado(
                  encargadosFiltrados[0]?.idEjecutivo || null
                );
