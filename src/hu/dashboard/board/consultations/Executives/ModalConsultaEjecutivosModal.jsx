...
const ModalConsultaEjecutivosModal = ({ isOpen, onClose }) => {
    ...
    const handleBuscar = async () => {
        ...
        try {
            ...
            console.log('ModalConsultaEjecutivos - userData from localStorage:', userData);
            ...
            if (encargado) {
                ...
                let match = (encargadosOptions || []).find(e => e.Usuario && String(e.Usuario).toUpperCase() === val.toUpperCase());
                if (!match) {
                    // Buscar por NombreEjecutivo
                    match = (encargadosOptions || []).find(e => String(e.NombreEjecutivo ?? e.nombreEjecutivo ?? e.nombre) === val);
                }
                if (!match) {
                    // Buscar por idEjecutivo
                    match = (encargadosOptions || []).find(e => String(e.idEjecutivo ?? '') === val);
                }
                if (match && match.Usuario) {
                    ...
                    encargadoToSend = match.Usuario ? String(match.Usuario).toUpperCase() : (match.idEjecutivo ? Number(match.idEjecutivo) : String(match.NombreEjecutivo ?? val));
                } else {
                ...
                if (userData?.usuario) encargadoToSend = String(userData.usuario).toUpperCase();
                else if (userData?.idEjecutivo) encargadoToSend = Number(userData.idEjecutivo);
                else encargadoToSend = null;
            }
            console.log('ModalConsultaEjecutivos - encargado resolved value (to send):', encargadoToSend);
            ...
            console.log('ModalConsultaEjecutivos - request body for ReportEjecutives:', body);
            // Log JSON stringified payload so we can see exactly what is sent over the wire
            try {
                console.log('ModalConsultaEjecutivos - Enviando ReportEjecutives payload (JSON):', JSON.stringify(body, (k, v) => (v === undefined ? null : v), 2));
            } catch (jsonErr) {
                console.warn('No se pudo serializar el body a JSON para logging:', jsonErr);
            }
            ...
            try {
                ...
                const ddByEjecutivo = (dropdownsEncargados || []).reduce((acc, d) => {
                    const didRaw = getVal(d, 'idEjecutivo', 'idEj', 'idEje', 'id_ejecutivo');
                    const did = (didRaw === undefined || didRaw === null) ? null : Number(didRaw);
                    if (!did) return acc;
                    if (!acc[did]) acc[did] = [];
                    acc[did].push(d);
                    return acc;
                ...
                const candidates = (rawJerarquia || []).map(item => {
                    const idEje = Number(getVal(item, 'idEjecutivo', 'idejecutivo', 'idEj', 'id') ?? 0) || 0;
                    const nombre = getVal(item, 'NombreEjecutivo', 'nombreEjecutivo', 'nombre') ?? null;
                    ...
                    const reportNames = new Set((data || []).map(r => normalize(getVal(r, 'NombreEjecutivo', 'nombreEjecutivo', 'nombre') ?? '')).filter(Boolean));

                    const candidatesWithNorm = candidates.map(c => ({ ...c, nombreNorm: normalize(c.nombre) }));
                    ...
                    console.log('Diagnostic counts: ddByEjecutivo keys=', Object.keys(ddByEjecutivo).length, 'candidates=', candidates.length, 'reportRows=', (data || []).length, 'reportNames=', reportNames.size, 'finalOptions=', finalOptions.length);
                    console.log('Encargados finalOptions from ReportEjecutives cross-check:', finalOptions.slice(0, 20));

                    // Construir entrada del ejecutivo de sesión (mostrar NombreEjecutivo pero cuando se envíe, se usará Usuario)
                    const sessionNombre = (userData?.NombreEjecutivo ?? userData?.nombreEjecutivo ?? userData?.nombre ?? userData?.usuario) || '';
                    const sessionUsuario = userData?.usuario ? String(userData.usuario).toUpperCase() : null;
                    const sessionIdEj = userData?.idEjecutivo ? Number(userData.idEjecutivo) : null;
                    const sessionEntry = { NombreEjecutivo: sessionNombre || (sessionUsuario || (sessionIdEj ? String(sessionIdEj) : '')), Usuario: sessionUsuario, idEjecutivo: sessionIdEj };
                    ...
                    const includesSession = (arr) => {
                        ...
                        const sUsuario = sessionEntry.Usuario ? String(sessionEntry.Usuario).toUpperCase() : null;
                        const sNombreNorm = sessionEntry.NombreEjecutivo ? normalize(sessionEntry.NombreEjecutivo) : null;
                        return arr.some(a => {
                            const aUsuario = a.Usuario ? String(a.Usuario).toUpperCase() : null;
                            const aNombreNorm = a.NombreEjecutivo ? normalize(a.NombreEjecutivo) : null;
                            if (sUsuario && aUsuario && sUsuario === aUsuario) return true;
                            if (sNombreNorm && aNombreNorm && sNombreNorm === aNombreNorm) return true;
                            return false;
                        });
                    };
                    ...
                    if (finalOptions.length === 0 && candidatesWithNorm.length > 0) {
                        ...
                        const fallback = candidatesWithNorm.map(c => ({ NombreEjecutivo: c.nombre, Usuario: c.usuario }));
                        // Normalizar Usuario a mayúsculas
                        let normalizedFallback = fallback.map(a => ({ ...a, Usuario: a.Usuario ? String(a.Usuario).toUpperCase() : null }));
                        // Asegurar que el ejecutivo de sesión esté como primera opción
                        if (!includesSession(normalizedFallback) && (sessionEntry.NombreEjecutivo || sessionEntry.Usuario)) {
                            normalizedFallback.unshift({ NombreEjecutivo: sessionEntry.NombreEjecutivo, Usuario: sessionEntry.Usuario ? String(sessionEntry.Usuario).toUpperCase() : null });
                        }
                        setEncargadosOptions(normalizedFallback);
                    } else if (finalOptions.length > 0) {
                        // Normalizar Usuario a mayúsculas
                        let normalizedFinal = finalOptions.map(a => ({ ...a, Usuario: a.Usuario ? String(a.Usuario).toUpperCase() : null }));
                        // Asegurar que el ejecutivo de sesión esté como primera opción
                        if (!includesSession(normalizedFinal) && (sessionEntry.NombreEjecutivo || sessionEntry.Usuario)) {
                            normalizedFinal.unshift({ NombreEjecutivo: sessionEntry.NombreEjecutivo, Usuario: sessionEntry.Usuario ? String(sessionEntry.Usuario).toUpperCase() : null });
                        }
                        setEncargadosOptions(normalizedFinal);
                    }
            } catch (crossErr) {
                console.warn('Error construyendo opciones de encargados desde ReportEjecutives:', crossErr);
            }
        } catch (err) {
            console.error(err);
    ...
    const handleExportar = () => {
        ...
        const headers = (visibleColumns && Array.isArray(visibleColumns) && visibleColumns.length > 0)
            ? visibleColumns
    ...
    const applyIndicadorFilter = (indicadorValue, allColumns) => {
        ...
        switch (indicadorValue) {
            ...
            case 'Negociaciones':
                ...
                if (cols.length >= 23) return cols.filter((c, i) => (i <= 3) || (i >= 16 && i <= 22));
                return null;

            case 'Búsquedas':
                // C#: ocultaba i>3 && i<23 || i>24 -> muestra resto
                if (cols.length >= 25) return cols.filter((c, i) => !(i > 3 && i < 23) && !(i > 24));
                return null;

            case 'Metas': {
                // Mostrar un conjunto específico por nombre cuando exista, si no, intentar por índices
                const metasNames = [
                    'CuentasGestionadas','MetaCuentas','%Cuentas',
                    'Titulares','MetaTitulares','%Titulares',
                    'Negociaciones','MetaNegociaciones','%Negociaciones',
                    'Cumplidas','MetaCumplimientos','%Cumplimientos',
                    'MontosCumplidos','MetaMontoCumplido','%MontoCumplido',
                    'SaldoSolucionado','MetaSaldoSolucionado','%SaldoSolucionado'
                ];
    ...
    const primaryOrder = ['nombreEjecutivo', 'ejecutivo', 'nombreEncargado', 'encargado'];
        ...
        if (!cols || cols === null) {
            ...
            const restFiltered = colsList.filter(c => !primariesPresent.includes(c));
        ...
        const loadEncargados = async () => {
            try {
                ...
                try {
                    ...
                    console.warn('No se pudo obtener obetenerDropdownsEncargados, proceder sin product-filter:', e);
                    dd = [];
                }

                // Log de diagnóstico (muestras)
                console.log('rawJerarquia length:', lista.length, 'dropdownsEncargados length:', (dd || []).length);
                if (lista && lista.length > 0) console.log('rawJerarquia sample keys:', Object.keys(lista[0]));
                if (dd && dd.length > 0) console.log('dropdownsEncargados sample keys:', Object.keys(dd[0]));
                ...
                console.error('Error cargando jerarquía de encargados o dropdowns:', err);
            }
        };
        loadEncargados();
    }, [isOpen]);
    ...
    useEffect(() => {
        ...
        const productoSelected = (sessionProduct && sessionProduct > 0)
            ? sessionProduct
            : ((producto && !isNaN(Number(producto))) ? Number(producto) : 0);
        ...
        console.log('sessionProduct:', sessionProduct, 'productoSelected:', productoSelected);

        const filtered = (rawJerarquia || []).filter(item => {
            const jer = Number(getVal(item, 'Jerarquía', 'Jerarquia', 'jerarquia', 'jer')) || 0;
            const idAreaRaw = getVal(item, 'idÁrea', 'idArea', 'idarea');
            const idArea = (idAreaRaw === undefined || idAreaRaw === null) ? null : (Number(idAreaRaw) || 0);
            const idEnc = Number(getVal(item, 'idEncargado', 'idencargado', 'idEnc', 'id_encargado')) || 0;
            ...
            if (dropdownsEncargados && dropdownsEncargados.length > 0) {
                ...
                const matches = dropdownsEncargados.filter(d => {
                    ...
                    return String(did).trim() === String(idEje).trim() || Number(did) === idEje;
                });
                if (matches.length === 0) {
                ...
                if (sessionProduct && sessionProduct > 0) {
                    const prodMatchSession = matches.some(d => {
                        ...
                        return Number(pid) === sessionProduct || String(pid).trim() === String(sessionProduct).trim();
                    });
                    ...
                    const prodMatch = matches.some(d => {
                        const pid = getVal(d, 'idProducto', 'idProd', 'id_producto');
                        if (pid === null || pid === undefined) return true;
                        return Number(pid) === productoSelected || String(pid).trim() === String(productoSelected).trim();
                    });
        ...
        console.log('Computed filtered encargados:', filtered.length, 'from rawJerarquia:', rawJerarquia.length, 'with dropdowns:', dropdownsEncargados.length);
        ...
        if (filtered.length === 0) {
            try {
                const diag = (rawJerarquia || []).slice(0, 20).map(item => {
                    ...
                    const matches = (dropdownsEncargados || []).filter(d => {
                        const did = getVal(d, 'idEjecutivo', 'idEj', 'idEje', 'id_ejecutivo');
                        return did !== undefined && (String(did).trim() === String(idEjeVal).trim() || Number(did) === idEjeVal);
                    });
                    const prods = Array.from(new Set(matches.map(m => getVal(m, 'idProducto', 'idProd', 'id_producto')))).slice(0, 10);
                    return { idEjecutivo: idEjeVal, NombreEjecutivo: getVal(item, 'NombreEjecutivo', 'nombreEjecutivo', 'nombre'), matchesInDropdowns: matches.length, sampleProducts: prods, idAreaRaw: idAreaRawLocal };
                });
                try {
                    console.log('Diagnóstico filtrado encargados (primeras 20 filas):', JSON.stringify(diag, null, 2));
                } catch {
                    console.log('Diagnóstico filtrado encargados (primeras 20 filas):', diag);
                }
            } catch (dErr) {
        ...
        let normalized = (filtered || []).map(it => ({
            ...
            NombreEjecutivo: it.NombreEjecutivo ?? it.nombreEjecutivo ?? it.nombre ?? ''
        }));

        // Asegurar que la entrada del ejecutivo de sesión esté presente como primera opción
        const sessionUsuario = userData?.usuario ? String(userData.usuario).toUpperCase() : null;
        const sessionNombre = userData?.NombreEjecutivo ?? userData?.nombreEjecutivo ?? userData?.nombre ?? userData?.usuario ?? '';
        const sessionOption = { NombreEjecutivo: sessionNombre || (sessionUsuario || ''), Usuario: sessionUsuario };
        const existsSession = normalized.some(n => {
            ...
            if (!sessionUsuario && sessionNombre && nName && sessionNombre === nName) return true;
            return false;
        });
        if (!existsSession && (sessionOption.NombreEjecutivo || sessionOption.Usuario)) {
            normalized.unshift(sessionOption);
        }

        console.log('encargadosOptions -> normalized sample:', normalized.slice(0, 20).map(x => ({ Usuario: x.Usuario, NombreEjecutivo: x.NombreEjecutivo, idEjecutivo: x.idEjecutivo })));
    ...
    const formatCellByKey = (h, v) => {
        ...
        const moneyKeys = new Set(['montonegociado','saldoasolucionar','montoscumplidos','montospagados','saldosolucionado','metamontocumplido','metasaldosolucionado']);

        // Porcentajes: columnas que contienen '%' o nombres claros
        const isPercentKey = headerKey.includes('porcentaje') || headerKey.includes('%') || headerKey.startsWith('pct') || headerKey.includes('meta') && headerKey.includes('%');

        // Formateadores locales
        const nfCurrency = new Intl.NumberFormat('es-ES', { minimumFractionDigits: 0, maximumFractionDigits: 2, useGrouping: true });
        const nfNumber = new Intl.NumberFormat('es-ES', { maximumFractionDigits: 6, useGrouping: true });

        // Si es campo monetario
        if (moneyKeys.has(headerKey) || headerKey.includes('monto') || headerKey.includes('saldo')) {
            ...
            if (isNumericString) {
                ...
                if (Math.abs(num) > 0 && Math.abs(num) < 1) return '$' + nfNumber.format(num);
    ...
    return (
        ...
        >
            <div className="coorin-modal-xl-container" style={{ maxWidth: "98vw", overflowX: "hidden" }}>
                ...
                <div className="flex items-center gap-6 mb-4 mt-4 px-4">
                    ...
                    <form className="flex-1 space-y-3">
                        ...
                        <div className="grid grid-cols-5 gap-2 w-full min-w-0">
                                ...
                                >
                                    ...
                                    {encargadosOptions && encargadosOptions.length > 0 ? (
                                        encargadosOptions.map((item, idx) => (
                                            <option key={(item.NombreEjecutivo ?? item.nombre ?? idx)} value={String(item.Usuario ?? item.NombreEjecutivo ?? item.nombre ?? item.idEjecutivo ?? '')}>
                                                {item.NombreEjecutivo ?? item.nombreEjecutivo ?? item.nombre}
                                            </option>
                                        ))
                                    ) : (
                                        <></>
                            ...
                            {/* Desde moved here (replacing Encargado) */}
                        ...
                        {/* Row 2: 5 columnas con Indicador y botones (Indicador col1, Buscar col2, Exportar col4) */}
                        <div className="grid grid-cols-5 gap-2 w-full min-w-0 mt-2">
                            {/* Indicador - column 1 */}
                            <div className="relative w-full col-start-1">
                                <select
                                    className="peer p-4 pe-9 block w-full bg-gray-50 border-transparent rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-jerarquia2 focus:border-jerarquia2 disabled:opacity-50 disabled:pointer-events-none focus:pt-6 focus:pb-2 not-placeholder-shown:pt-6 not-placeholder-shown:pb-2 autofill:pt-6 autofill:pb-2"
                                    value={indicador}
                                    onChange={(e) => setIndicador(e.target.value)}
                                    id="indicador-select-row2"
                                >
                                    <option value="" disabled hidden></option>
                            ...
                            {/* Buscar - column 2 centered */}
                            <div className="col-start-2 flex justify-center items-end">
                                <button
                                    type="button"
                                    className="btn-success sm:w-auto min-w-[120px] px-6 py-2 text-base font-medium rounded-lg shadow-sm flex justify-center"
                            ...
                            {/* Exportar - column 4 centered */}
                            <div className="col-start-4 flex justify-center items-end">
                                <button
                                    className="btn-info sm:w-auto min-w-[120px] px-6 py-2 text-base font-medium rounded-lg shadow-sm flex justify-center"
                                    type="button"
                ...
                {/* Tabla de resultados */}
                <div className="overflow-x-auto overflow-y-scroll relative" style={{ maxHeight: "600px", overflowY: 'scroll' }}>
                    ...
                    {loading && (
                        <div className="absolute inset-0 flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.6)', zIndex: 90 }}>
                            <div className="flex flex-col items-center">
                                <div className="animate-spin rounded-full h-16 w-16 border-t-4 border-jerarquia3" style={{ borderRightColor: 'transparent' }}></div>
                                <div className="mt-2 text-sm text-gray-700">Cargando resultados...</div>
                            </div>
                        </div>
                    )}
                            ...
                            <tr>
                                {resultados && resultados.length > 0 ? (
                                    (visibleColumns || Object.keys(resultados[0])).map((h) => {
                                        ...
                                        const secondWidth = 200; // Ejecutivo
                                        const thStyle = {};
                                    ...
                                    <>
                                            <th className="whitespace-nowrap" style={{ position: 'sticky', top: 0, left: 0, zIndex: 70, background: 'var(--color-background-secondary)', color: '#fff', fontWeight: 600, minWidth: '240px' }}>NombreEjecutivo</th>
                                        <th className="whitespace-nowrap" style={{ position: 'sticky', top: 0, left: 240, zIndex: 60, background: 'var(--color-background-secondary)', color: '#fff', fontWeight: 600, minWidth: '200px' }}>Ejecutivo</th>
                                ...
                                <tr key={idx} onClick={() => setSelectedRowKey(idx)} style={{ cursor: 'pointer' }}>
                                    {(visibleColumns || Object.keys(row)).map((k) => {
                                        ...
                                        if (isStickyNombre) {
                                            ...
                                            tdStyle.background = (selectedRowKey === idx) ? 'var(--color-jerarquia3)' : '#fff';
                                            if (selectedRowKey === idx) tdStyle.color = '#fff';
                                            tdStyle.minWidth = firstWidth + 'px';
                                            tdStyle.borderRight = '1px solid var(--color-jerarquia3)';
                                            tdClass += ' border-r';
                                        } else if (isStickyEjecutivo) {
                                            tdStyle.position = 'sticky';
                                        ...
                                        if (!isStickyNombre && !isStickyEjecutivo && selectedRowKey === idx) {
                                            tdStyle.background = 'var(--color-jerarquia3)';
                                            tdStyle.color = '#fff';
                                        }
                                        return (
                                            <td key={k + idx} className={tdClass} style={tdStyle}>{formatCellByKey(k, row[k])}</td>
                                        );
                                    })}
                                </tr>
                            ))}
