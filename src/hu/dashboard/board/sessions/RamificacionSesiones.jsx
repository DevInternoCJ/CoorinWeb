  ...
  const renderExecutiveTree = (tree, level = 0, parentKey = "") => {
    ...
    return tree.map((node, idx) => {
      ...
      return (
        <div
          key={nodeKey}
          className={`hs-accordion hs-dragged:bg-blue-100 hs-dragged:rounded nested-2-${
            level + 1
          }${isSelected ? " hs-tree-view-selected:bg-gray-100" : ""}`}
          role="treeitem"
              ...
              <div
                className={`sticky-session-executive${
                  selectedExecutiveNode === Number(idEjecutivoSesion)
                    ? " selected"
                    : ""
                } text-xs px-2 py-1 rounded-md bg-gray-50 border text-center max-w-full truncate`}
