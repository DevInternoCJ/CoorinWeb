...
const Logout = ({
    ...
    const handleLogout = async () => {
        try {
            // Preparar los datos para el logout según lo que pide el endpoint
            const logoutData = {
            ...
            // Continuamos con el proceso aunque el endpoint falle
        } finally {
            // SIEMPSE ejecutamos estas acciones
            // 1. Limpiar el store global
            logout();
            
            // 2. Ejecutar la función onLogout si fue proporcionada como prop
    ...
    return (
        <div className="hs-dropdown [--strategy:absolute] [--auto-close:inside] relative w-full inline-flex">
            ...
            >
                ...
                {/* Nombre del usuario: Se oculta cuando la sidebar está minificada */}
                ...
                {/* Icono de flecha: Se oculta cuando la sidebar está minificada */}
                <svg
            ...
            >
                <div className="p-1">
                    {/* Opción 1: Rol/Cargo del usuario */}
                    <div
                        className="flex items-center gap-x-3 py-2 px-3 rounded-lg text-sm text-gray-800 disabled:opacity-50 disabled:pointer-events-none focus:outline-hidden focus:bg-gray-100"
                    >
                        ID: {idEjecutivo}
                    </div>
