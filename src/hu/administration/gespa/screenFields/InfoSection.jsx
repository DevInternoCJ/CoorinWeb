...
import { ShowFieldScreen } from "../../../../services/LokiServices";
  ...
  <table className="table-border border border-jerarquia1 rounded-lg text-background-dashboard w-full">
    <tbody>
      {Array.from({ length: 5 }).map((_, rowIdx) => {
        ...
        return (
          <tr key={rowIdx} className="border-b border-jerarquia4 last:border-b-0">
            {rowItems.map((item, colIdx) => (
              <React.Fragment key={colIdx}>
                <td className="py-2 px-2 font-semibold">{item?.label || ""}</td>
                <td
                  className="py-2 px-2"
                  style={{
                    fontWeight: item?.fontWeight === "font-weight-bold" ? "bold" : "normal",
