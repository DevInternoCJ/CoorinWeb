...
namespace Loki.Mark.Captura.Visitas.Services
{
    public class CapturaVisitasService : ICapturaVisitasService
    {
        ...
        /// </summary>
        public async Task<(bool Exitoso, string Mensaje)> GuardarVisitaAsync(string servidor, CapturaVisitaRequestDto request)
        {
            // --- 1. Validaciones ---
            ...
            int idEjecutivoClaim = request.IdEjecutivo;
            if (idEjecutivoClaim == 0)
            {
                return (false, "No se pudo obtener el ID del ejecutivo.");
            }
            ...
            var gestionResponse = await _dao.GuardarGestionDomiciliariaAsync(servidor, request, idEjecutivoClaim);
