...
namespace Loki.Mark.Administracion.Ejecutivos.Encargados.Services
{
	public class EncargadosService : IEncargadosService
	{
		...
		public async Task<bool> CambiarEncargado(string servidor, CambiaEncargadoDto cambiaEncargado)
		{
			using var connection = _dbContFactory.GetSqlConnection(servidor, "Collection");

			var pootis = await _encargadosDao.CambiaEncargadoEjecutivo(servidor, cambiaEncargado);


			return pootis.FirstOrDefault();
