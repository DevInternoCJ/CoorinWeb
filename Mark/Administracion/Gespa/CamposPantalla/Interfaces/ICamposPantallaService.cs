...
namespace Loki.Mark.Administracion.Gespa.CamposPantalla.Interfaces
{
	public interface ICamposPantallaService
	{
		...
		Task<IEnumerable<dynamic>> GridProductoTableSample(string servidor, int idProducto, double porcentaje = 70, int? maximo = 5);
		Task<GuardarCamposResult> InsertaActualizaCamposPantalla(string servidor, CampoPantallaRequest request);
	}
}