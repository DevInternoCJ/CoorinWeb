...
namespace Loki.Mark.Administracion.Gespa.CamposPantalla.Controllers
{
	...
	{
		private readonly CamposPantallaService _campService;
		public CamposPantallaController(CamposPantallaService service)
		{
		...
		[HttpGet("grid-producto-sample/{servidor}/{idProducto}")]
		[SwaggerOperation(
		...
		public async Task<IActionResult> GridProductoSample(string servidor, int idProducto, [FromQuery] double porcentaje = 70, [FromQuery] int? maximo = 5)
		{
			var existe = await _campService.ObtenerObjectIdProductoAsync(servidor, idProducto);
			if (!existe.HasValue)
