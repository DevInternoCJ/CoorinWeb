using System;
using System.Collections.Generic;

namespace Loki.AsuraLibrary.ModelsHistory;

public partial class AcumProducto14
{
    public DateOnly? Insert { get; set; }

    public int IdLogAsignación { get; set; }

    public string? CarteraAl { get; set; }

    public string? IdPersona { get; set; }

    public string IdCuenta { get; set; } = null!;

    public string? Column1 { get; set; }

    public string? NombreCliente { get; set; }

    public string? CalleNo { get; set; }

    public string? Colonia { get; set; }

    public string? Cp { get; set; }

    public string? Poblacion { get; set; }

    public string? EntFed { get; set; }

    public string? Rfc { get; set; }

    public string? TelefonoCte1 { get; set; }

    public string? TelefonoCte2 { get; set; }

    public string? TelefonoCte3 { get; set; }

    public string? NombreAval { get; set; }

    public string? CalleNoA { get; set; }

    public string? ColoniaA { get; set; }

    public string? Cpa { get; set; }

    public string? PoblacionA { get; set; }

    public string? EntFedA { get; set; }

    public string? TelefonoAval { get; set; }

    public string? SaldoVencido { get; set; }

    public string? Otros { get; set; }

    public string? FecMasAntiguo { get; set; }

    public string? NoRtasVencidas { get; set; }

    public string? MontoRenta { get; set; }

    public string? Plazo { get; set; }

    public string? RtaMasAntigua { get; set; }

    public string? DiasVencido { get; set; }

    public string? Producto { get; set; }

    public string? Ccpersona { get; set; }

    public string? NomDealer { get; set; }

    public string? IdSucDealer { get; set; }

    public string? Buro { get; set; }

    public string? Riesgo { get; set; }

    public string? Despacho { get; set; }

    public string? Campaña { get; set; }

    public string? BuroAlDiaVerificacion { get; set; }

    public string? BuróAsignación { get; set; }

    public string? Diasvenc { get; set; }

    public string? Fechamasantigua { get; set; }

    public string? Numpagosven { get; set; }

    public string? Programa { get; set; }

    public string? Tipopersona { get; set; }

    public string? Totalotros { get; set; }

    public string? Totalvencidos { get; set; }

    public string? ÚltimaGestión { get; set; }

    public string? Proyecto { get; set; }

    public string? Fila { get; set; }

    public string? _120 { get; set; }

    public string? _3060 { get; set; }

    public string? _6090 { get; set; }

    public string? _90120 { get; set; }

    public string? Ene30 { get; set; }

    public string? Asignado { get; set; }

    public string? _41 { get; set; }

    public string? _60901 { get; set; }

    public string? _24 { get; set; }

    public string? _29 { get; set; }

    public string? Numpagosven2 { get; set; }

    public string? _0130 { get; set; }

    public string? _1201 { get; set; }

    public string? Despachos { get; set; }

    public string? _28 { get; set; }

    public string? ParaProyecto1 { get; set; }

    public string? Fechaproceso { get; set; }

    public string? Clave { get; set; }

    public string? CargosACondonar { get; set; }

    public string? Relacionadas { get; set; }

    public string? AsigandoA { get; set; }

    public string? Tipo { get; set; }

    public string? Tfsm { get; set; }

    public string? Personalizada { get; set; }

    public string? Probabilidad { get; set; }

    public string? Riesgo1 { get; set; }

    public string? Prioridad { get; set; }

    public string? Asigando { get; set; }

    public string? CampoAdicional { get; set; }

    public string? Personalizadas { get; set; }

    public string? Uber { get; set; }

    public string? DessCargos { get; set; }

    public string? DiasPago { get; set; }

    public string? EquipoCall { get; set; }

    public string? VectorPagos { get; set; }

    public string? DiasDePago { get; set; }

    public string? Corte { get; set; }

    public string? AnalisisCuenta { get; set; }

    public string? FecMasAnti { get; set; }

    public string? F38 { get; set; }

    public string? F39 { get; set; }

    public string? SaldoPago { get; set; }

    public string? Bucket { get; set; }

    public string? Gestiones { get; set; }

    public string? MaxGest { get; set; }

    public string? MinGest { get; set; }

    public string? Score { get; set; }

    public string? Estrategia { get; set; }

    public string? Marca { get; set; }

    public string? Asigando1 { get; set; }
}
