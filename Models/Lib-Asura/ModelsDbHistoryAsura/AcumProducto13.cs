using System;
using System.Collections.Generic;

namespace Loki.AsuraLibrary.ModelsHistory;

public partial class AcumProducto13
{
    public DateOnly? Insert { get; set; }

    public int IdLogAsignación { get; set; }

    public string? F1 { get; set; }

    public string IdCuenta { get; set; } = null!;

    public string? NameS { get; set; }

    public string? Ordner { get; set; }

    public string? Ordnerdat { get; set; }

    public string? Ordnerpre { get; set; }

    public string? Saldo { get; set; }

    public string? Hoy { get; set; }

    public string? AgNvaAsignacion { get; set; }

    public string? MesMasAntiguo { get; set; }

    public string? Hoy1 { get; set; }

    public string? DiasAtraso { get; set; }

    public string? UltimoPago { get; set; }

    public string? ImporteUltPago { get; set; }

    public string? Estado { get; set; }

    public string? Behaviour { get; set; }

    public string? ClienteDesde { get; set; }

    public string? Curp { get; set; }

    public string? Empresa { get; set; }

    public string? FNacimiento { get; set; }

    public string? InicioCto { get; set; }

    public string? ObligadoSolidario { get; set; }

    public string? Plazo { get; set; }

    public string? ReferenciaPago { get; set; }

    public string? VtoCto { get; set; }

    public string? Rfc { get; set; }

    public string? ClienteDesde1 { get; set; }

    public string? C { get; set; }

    public string? ClienteDesde2 { get; set; }

    public string? Con { get; set; }

    public string? CxC { get; set; }

    public string? Cliente1 { get; set; }

    public string? FechaUltimoPago { get; set; }

    public string? ImporteUltPago1 { get; set; }

    public string? CarpetaAyer { get; set; }

    public string? Etiqueta { get; set; }

    public string? FacturaMasAntigua { get; set; }

    public string? Risk { get; set; }

    public string? Nafin { get; set; }

    public string? F20 { get; set; }

    public string? F21 { get; set; }

    public string? F22 { get; set; }

    public string? Nombre { get; set; }

    public string? Carpeta { get; set; }

    public string? FechaDeAlta { get; set; }

    public string? FechaDeAsignacion { get; set; }

    public string? CarpetaAnterior { get; set; }

    public string? SaldoCarteraVencida { get; set; }

    public string? CarpetaHoy { get; set; }

    public string? AgenciaDeCobranzaAyer { get; set; }

    public string? AgenciaDeCobranzaHoy { get; set; }

    public string? FechaDeHoy { get; set; }

    public string? DiasDeAtraso { get; set; }

    public string? ImporteUltimoPago { get; set; }

    public string? CuentaPorCobrar { get; set; }

    public string? Uber { get; set; }

    public string? CampoAdicional { get; set; }

    public string? Contingencia { get; set; }

    public string? Tlg { get; set; }

    public string? CarpetaDeHoy { get; set; }

    public string? Reest { get; set; }

    public string? Agencia { get; set; }

    public string? Agencia1 { get; set; }

    public string? Año { get; set; }

    public string? EstadoAuto { get; set; }

    public string? Marca { get; set; }

    public string? Modelo { get; set; }

    public string? NoMotor { get; set; }

    public string? NoSerie { get; set; }

    public string? Agencia2 { get; set; }

    public string? FechaDeAsignacionAgencia { get; set; }

    public string? RiesgoDeTraspaso { get; set; }

    public string? PlanDeApoyo { get; set; }

    public string? DateOfBirth { get; set; }

    public string? Minimo { get; set; }

    public string? OtrosConceptos { get; set; }

    public string? Tipo { get; set; }

    public string? AutoNuevoUsado { get; set; }

    public string? CreditPagoFinalVencido { get; set; }

    public string? PagoFinal { get; set; }

    public string? CarpetaHoy1 { get; set; }

    public string? Segmento { get; set; }
}
