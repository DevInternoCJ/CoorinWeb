using System;
using System.Collections.Generic;

namespace Loki.AsuraLibrary.ModelsHistory;

public partial class AcumProducto28
{
    public DateOnly? Insert { get; set; }

    public int IdLogAsignación { get; set; }

    public string? TipoQuita { get; set; }

    public string? Quita { get; set; }

    public string? QuitaPlaz { get; set; }

    public string? Marca { get; set; }

    public string? MCliente { get; set; }

    public string? Cartera { get; set; }

    public string? Despacho { get; set; }

    public string? FecAsig { get; set; }

    public string? Tipo { get; set; }

    public string? MarcaCs { get; set; }

    public string? Prioridad { get; set; }

    public string? Prestamo { get; set; }

    public string IdCuenta { get; set; } = null!;

    public string? Nombre { get; set; }

    public string? Rfc { get; set; }

    public string? MoraForz { get; set; }

    public string? MContIni { get; set; }

    public string? CartIni { get; set; }

    public string? SdoContI { get; set; }

    public string? MAplcIni { get; set; }

    public string? SdoAplcI { get; set; }

    public string? CartAct { get; set; }

    public string? MContAct { get; set; }

    public string? SdoContA { get; set; }

    public string? MAplcAct { get; set; }

    public string? SdoAplcA { get; set; }

    public string? SdoVen { get; set; }

    public string? PagMin { get; set; }

    public string? PagoFijo { get; set; }

    public string? Diasmora { get; set; }

    public string? LimCred { get; set; }

    public string? Portafolio { get; set; }

    public string? Producto { get; set; }

    public string? Status { get; set; }

    public string? Status2 { get; set; }

    public string? FechaCs { get; set; }

    public string? Ciclo { get; set; }

    public string? MtoUlPg { get; set; }

    public string? FUltPago { get; set; }

    public string? NumCte { get; set; }

    public string? CalleNum { get; set; }

    public string? Colonia { get; set; }

    public string? Poblacion { get; set; }

    public string? Cp { get; set; }

    public string? Estado { get; set; }

    public string? Tel1 { get; set; }

    public string? Tel2 { get; set; }

    public string? Tel3 { get; set; }

    public string? Tel4 { get; set; }

    public string? Tel5 { get; set; }

    public string? Tel6 { get; set; }

    public string? Tel7 { get; set; }

    public string? Tel8 { get; set; }

    public string? Tel9 { get; set; }

    public string? Tel10 { get; set; }

    public string? Plaza { get; set; }

    public string? Region { get; set; }

    public string? Division { get; set; }

    public string? Nodo { get; set; }

    public string? CtroResp { get; set; }

    public string? PrestCoor { get; set; }

    public string? MCoorpI { get; set; }

    public string? MCoorpA { get; set; }

    public string? SdoCorpI { get; set; }

    public string? SdoCorpA { get; set; }

    public string? Bucket1 { get; set; }

    public string? Bucket2 { get; set; }

    public string? Bucket3 { get; set; }

    public string? Bucket4 { get; set; }

    public string? Bucket5 { get; set; }

    public string? Bucket6 { get; set; }

    public string? Bucket7 { get; set; }

    public string? Nomdug { get; set; }

    public string? Divisional { get; set; }

    public string? McaDemand { get; set; }

    public string? Abanderad { get; set; }

    public string? FhCapCyb { get; set; }

    public string? ComenCyb { get; set; }

    public string? FhCapCc { get; set; }

    public string? UltimGes { get; set; }

    public string? ComenCc { get; set; }

    public string? Ca1 { get; set; }

    public string? Cr1 { get; set; }

    public string? Ca2 { get; set; }

    public string? Cr2 { get; set; }

    public string? Ca3 { get; set; }

    public string? Cr3 { get; set; }

    public string? Correo { get; set; }

    public string? Enc { get; set; }

    public string? Cr { get; set; }

    public string? CartIni2 { get; set; }

    public string? CartAct2 { get; set; }

    public string? Exclusion { get; set; }

    public string? Bandera { get; set; }

    public string? Statpres { get; set; }

    public string? Dosplazas { get; set; }

    public string? Gestionabl { get; set; }

    public string? PlazaCte { get; set; }

    public string? CerCte { get; set; }

    public string? DiviCte { get; set; }

    public string? Cobertura { get; set; }

    public string? DespReme { get; set; }

    public string? AsigReme { get; set; }

    public string? ExcQuita { get; set; }

    public string? Envio { get; set; }

    public string? CartIniC { get; set; }

    public string? Multiprod { get; set; }

    public string? Encargado { get; set; }

    public string? Scv { get; set; }

    public string? Fechaarchivopagos { get; set; }

    public string? SegEstrategia { get; set; }

    public string? VigVdaFall { get; set; }

    public string? FechaActualización { get; set; }

    public string? Municipio { get; set; }

    public string? Periferia { get; set; }

    public string? Sucursal { get; set; }

    public string? Estrategia { get; set; }

    public string? _ { get; set; }

    public string? Comentario { get; set; }

    public string? FechaLimAplicacion { get; set; }

    public string? PagoInicial { get; set; }

    public string? Statusvinculacion { get; set; }

    public string? DiasmIni2 { get; set; }

    public string? FecAplcBr { get; set; }

    public string? FechaVinculación { get; set; }

    public string? MinimoIni2 { get; set; }

    public string? MontoVinculación { get; set; }

    public string? MoraaplcI2 { get; set; }

    public string? MoracontI2 { get; set; }

    public string? SdocontI2 { get; set; }

    public string? Solicitud2 { get; set; }

    public string? StatusBr { get; set; }

    public string? VencidIni2 { get; set; }

    public string? MaxMora { get; set; }

    public string? CartActOk { get; set; }

    public string? Fecharef { get; set; }

    public string? Dmname { get; set; }

    public string? Grupo { get; set; }

    public string? UltFecAc { get; set; }

    public string? SdoTotCa { get; set; }

    public string? Segmento { get; set; }

    public string? CartIniCuadre { get; set; }

    public string? FecAsigCuadre { get; set; }

    public string? MoraAplcCuadre { get; set; }

    public string? FechaÚltimoPago { get; set; }

    public string? NúmeroCliente { get; set; }

    public string? Telefono1 { get; set; }

    public string? Telefono2 { get; set; }

    public string? Telefono3 { get; set; }

    public string? Telefono4 { get; set; }

    public string? Telefono5 { get; set; }

    public string? Telefono6 { get; set; }

    public string? Telefono7 { get; set; }

    public string? Telefono8 { get; set; }

    public string? Telefono9 { get; set; }

    public string? Telefono10 { get; set; }

    public string? UltimaFechaDeActividad { get; set; }

    public string? Mail { get; set; }

    public string? Cartera1 { get; set; }

    public string? Ucprodcrit { get; set; }

    public string? NumeroDeCliente { get; set; }

    public string? Rfc1 { get; set; }

    public string? Product { get; set; }

    public string? GrupoCyber { get; set; }

    public string? ProductoPyme { get; set; }

    public string? Demandado { get; set; }

    public string? OfertaDemandados { get; set; }

    public string? PagoMin { get; set; }

    public string? FechaDesvinculacionManual { get; set; }

    public string? FechaVinculacionManual { get; set; }

    public string? MontoVinculacionManual { get; set; }

    public string? NombreHelpDeskVinculacionManual { get; set; }

    public string? UsuarioVinculacionManual { get; set; }

    public string? ActLab { get; set; }

    public string? NegOp { get; set; }

    public string? NoCliente { get; set; }

    public string? OfertaDemandados1 { get; set; }

    public string? NombreCliente { get; set; }

    public string? Representante { get; set; }

    public string? Asignacion { get; set; }

    public string? Ejecutivo { get; set; }
}
