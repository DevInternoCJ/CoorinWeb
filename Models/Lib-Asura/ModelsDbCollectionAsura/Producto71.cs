using System;
using System.Collections.Generic;

namespace Loki.AsuraLibrary.ModelsCollection;

public partial class Producto71
{
    public string Idcuenta { get; set; } = null!;

    public string? NumLineas { get; set; }

    public string? Agencia { get; set; }

    public string? Ciclo { get; set; }

    public string? Bucket { get; set; }

    public string? Email { get; set; }

    public string? Motivo { get; set; }

    public string? FechaActivacion { get; set; }

    public string? FechaSusp { get; set; }

    public string? CapaSaldo { get; set; }

    public string? AreaConcepto { get; set; }

    public string? CreditScore { get; set; }

    public string? Vto { get; set; }

    public string? Region { get; set; }

    public string? Plaza { get; set; }

    public string? Tenure { get; set; }

    public string? RangoTenure { get; set; }

    public string? NoPagos { get; set; }

    public string? RangoNopagos { get; set; }

    public string? Edad { get; set; }

    public string? RangoEdad { get; set; }

    public string? VtoPlazoforzoso { get; set; }

    public string? RangoVtoplazof { get; set; }

    public string? Tecnologia { get; set; }

    public string? Datacard { get; set; }

    public string? TipoPlan { get; set; }

    public string? Score { get; set; }

    public string? Banamex { get; set; }

    public string? OtrosBancos { get; set; }

    public string? BttcTt { get; set; }

    public string? Phone1 { get; set; }

    public string? Phone2 { get; set; }

    public string? Phone3 { get; set; }

    public string? FechaCargoLineaAprobado { get; set; }

    public string? CalleFiscal { get; set; }

    public string? ColoniaFiscal { get; set; }

    public string? PoblacionFiscal { get; set; }

    public string? CpFiscal { get; set; }

    public string? CalleEnvio { get; set; }

    public string? ColoniaEnvio { get; set; }

    public string? PoblacionEnvio { get; set; }

    public string? CpEnvio { get; set; }

    public string? ClaveLada1 { get; set; }

    public string? ClaveLada2 { get; set; }

    public string? Telefono { get; set; }

    public string? Telefono2 { get; set; }

    public string? RepresentanteLegal { get; set; }

    public string? FechaEstadia { get; set; }

    public string? EtiquetaSegmento { get; set; }

    public string? FechaCancelacion { get; set; }

    public string? SaldoActual { get; set; }

    public string? Saldo130 { get; set; }

    public string? Saldo3160 { get; set; }

    public string? Saldo6190 { get; set; }

    public string? Saldo91120 { get; set; }

    public string? Saldo120Sup { get; set; }

    public string? SaldoTotal { get; set; }

    public string? SaldoVencido { get; set; }

    public string? MesSuspension { get; set; }

    public string? Bonificacion { get; set; }

    public string? SaldoVencidoCalculo { get; set; }

    public string? BonificacionTotal { get; set; }

    public string? PagoCliente { get; set; }

    public string? PorcentajePago { get; set; }

    public string? PrimerPago { get; set; }

    public string? EjecutivoVentas { get; set; }

    public string? Distribuidor { get; set; }

    public string? Segmento { get; set; }

    public string? DiasSusp { get; set; }

    public string? RegionNueva { get; set; }

    public string? Estado { get; set; }

    public string? SaldoCurrent { get; set; }

    public string? PlanTarifario { get; set; }

    public string? Etf { get; set; }

    public string? FecCambioBk { get; set; }

    public string? FecCambioBk2 { get; set; }

    public string? FecCambioBk3 { get; set; }

    public string? FecCambioPch { get; set; }

    public string? FecRetiro { get; set; }

    public string? Campaña { get; set; }

    public string? _5050 { get; set; }

    public string? SaldoCurrent1 { get; set; }

    public string? SaldoTot { get; set; }

    public string? Condonacion { get; set; }

    public string? Ofrecimiento { get; set; }
}
