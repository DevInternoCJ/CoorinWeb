using System;
using System.Collections.Generic;

namespace Loki.DbHistory.ModelsCronoss;

public partial class AcumProducto21
{
    public DateOnly? Insert { get; set; }

    public int IdLogAsignación { get; set; }

    public string? Cliente { get; set; }

    public string IdCuenta { get; set; } = null!;

    public string? Operación { get; set; }

    public string? Nombre { get; set; }

    public string? Empresa { get; set; }

    public string? Calle { get; set; }

    public string? Colonia { get; set; }

    public string? Ciudad { get; set; }

    public string? Poblacion { get; set; }

    public string? CodigoPostal { get; set; }

    public string? TelPrinc { get; set; }

    public string? TelEmp { get; set; }

    public string? TelOtros { get; set; }

    public string? DiaCorte { get; set; }

    public string? FechaDeApertura { get; set; }

    public string? Periodo { get; set; }

    public string? Mensualidad { get; set; }

    public string? PagoEspecial { get; set; }

    public string? MesPagoEspecial { get; set; }

    public string? PagoVencidoAntigüedad { get; set; }

    public string? SaldoVencido { get; set; }

    public string? SaldoInsoluto { get; set; }

    public string? SaldoTotal { get; set; }

    public string? Marca { get; set; }

    public string? Modelo { get; set; }

    public string? MotivoDeRechazo { get; set; }

    public string? Aval { get; set; }

    public string? DirecciónAvalOCotitular { get; set; }

    public string? MunicipioAvalOCotitular { get; set; }

    public string? EstadoAvalOCotitular { get; set; }

    public string? TelAval { get; set; }

    public string? TelAval2 { get; set; }

    public string? NomRef1 { get; set; }

    public string? TelRef1 { get; set; }

    public string? NomRef2 { get; set; }

    public string? TelRef2 { get; set; }

    public string? NomRef3 { get; set; }

    public string? TelRef3 { get; set; }

    public string? Estado { get; set; }

    public string? Empresa1 { get; set; }

    public string? DomEmpresa { get; set; }

    public string? Telefonos { get; set; }

    public string? CargoTitular { get; set; }

    public string? JefeInmediato { get; set; }

    public string? Rfc { get; set; }

    public string? NoSerie { get; set; }

    public string? Placas { get; set; }

    public string? DomActual { get; set; }

    public string? TelAdicioles { get; set; }

    public string? CelularNextel { get; set; }

    public string? DatosIvr { get; set; }

    public string? CElectronico { get; set; }

    public string? Riesgo { get; set; }

    public string? Nodo { get; set; }

    public string? NoAplicaRenegociación { get; set; }

    public string? Despacho { get; set; }

    public string? Status { get; set; }

    public string? SubStatus { get; set; }

    public string? Renegociacion { get; set; }

    public string? TerminalAmpliacion { get; set; }

    public string? StatusMesAnterior { get; set; }

    public string? StatusSeguro { get; set; }

    public string? SubStatusSeguro { get; set; }

    public string? ClaveDistribuidor { get; set; }

    public string? ApvVendedor { get; set; }

    public string? Enganche { get; set; }

    public string? FechaAsignación { get; set; }

    public string? Asignacion { get; set; }

    public string? ProyectoInplant { get; set; }

    public string? Cartera { get; set; }

    public string? PlantaEmpleados { get; set; }

    public string? Grupo { get; set; }

    public string? AgenciasDeAltoRiesgo { get; set; }

    public string? DTotCarteraContSoloLunesRc62CierreConPagos { get; set; }

    public string? Motor { get; set; }

    public string? Creditaxis { get; set; }

    public string? AmortizaciónActual { get; set; }

    public string? NombreDeTercero { get; set; }

    public string? DireccionTercero { get; set; }

    public string? MailsTercero { get; set; }

    public string? Tels { get; set; }

    public string? Autoaprobadas { get; set; }

    public string? Pva { get; set; }

    public string? Sva { get; set; }

    public string? Domi { get; set; }

    public string? SvaDomi { get; set; }

    public string? Formula { get; set; }

    public string? ConcentradoDeIngresos { get; set; }

    public string? Sva1 { get; set; }

    public string? Operación1 { get; set; }

    public string? ScoreDeCredito { get; set; }

    public string? Pva1 { get; set; }

    public string? F22 { get; set; }

    public string? Pvaerr { get; set; }

    public string? Svaerr { get; set; }

    public string? ScorerDeCredito { get; set; }

    public string? AutoAprobación { get; set; }

    public string? Daciones { get; set; }

    public string? Año { get; set; }

    public string? Resultado { get; set; }

    public string? Dp { get; set; }

    public string? ScoreDeRiesgo { get; set; }

    public string? F82 { get; set; }

    public string? RiskSegment { get; set; }

    public string? Svr { get; set; }

    public string? Status1 { get; set; }

    public string? Ajuste { get; set; }

    public string? BonificacionDeCapital { get; set; }

    public string? Terminal { get; set; }

    public string? DispositivoSatelital { get; set; }

    public string? SaldoTotal1 { get; set; }

    public string? Marca1 { get; set; }

    public string? Modelo1 { get; set; }

    public string? MotivoDeRechazo1 { get; set; }

    public string? PagoSostenido { get; set; }

    public string? Subprime { get; set; }

    public string? RenegociacionRe01 { get; set; }

    public string? Renegociacion6Meses { get; set; }

    public string? Uber { get; set; }

    public string? DSldContable { get; set; }

    public string? Dtotcarteracont { get; set; }

    public string? Renegociacion6Meses1 { get; set; }

    public string? Terminal1 { get; set; }

    public string? Uber1 { get; set; }

    public string? F87 { get; set; }

    public string? F88 { get; set; }

    public string? Herramienta { get; set; }

    public string? ProgramaIncentivoDacion { get; set; }

    public string? Incentivo { get; set; }

    public string? Campaña { get; set; }

    public string? Empleo { get; set; }

    public string? Segmento { get; set; }

    public string? F89 { get; set; }

    public string? F90 { get; set; }

    public string? CelularNextel1 { get; set; }

    public string? Opcion { get; set; }

    public string? Herramienta2 { get; set; }

    public string? Herramienta3 { get; set; }

    public string? Compactacion { get; set; }

    public string? Despacho1 { get; set; }

    public string? FaltanteHerramienta { get; set; }

    public string? Equipo { get; set; }

    public string? LocalForaneo { get; set; }

    public string? Ruta { get; set; }

    public string? Etiqueta { get; set; }

    public string? Estrategia { get; set; }

    public string? F20 { get; set; }

    public string? DescuentoEntero { get; set; }

    public string? PreWOffYPreMora { get; set; }

    public string? Product { get; set; }

    public string? StatusDemanda { get; set; }

    public string? Vigencia { get; set; }

    public string? AlcanceDeRecuperación { get; set; }

    public string? Multicliente { get; set; }

    public string? Beneficio { get; set; }

    public string? MoraInicial { get; set; }

    public string? Responsable { get; set; }

    public string? MortaInicial { get; set; }

    public string? FiltroDeReestructura { get; set; }
}
