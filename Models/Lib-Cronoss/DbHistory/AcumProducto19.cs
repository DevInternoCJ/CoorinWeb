using System;
using System.Collections.Generic;

namespace Loki.DbHistory.ModelsCronoss;

public partial class AcumProducto19
{
    public DateOnly? Insert { get; set; }

    public int IdLogAsignación { get; set; }

    public string? Tipo { get; set; }

    public string? IdPersona { get; set; }

    public string? NombreCliente { get; set; }

    public string? Dealer { get; set; }

    public string? Plazo { get; set; }

    public string? RefCobro { get; set; }

    public string IdCuenta { get; set; } = null!;

    public string? TRealFinanciada { get; set; }

    public string? Capital { get; set; }

    public string? Enganche { get; set; }

    public string? Seguro { get; set; }

    public string? MontoFinanciado { get; set; }

    public string? FecRegistro { get; set; }

    public string? Fec1ªRenta { get; set; }

    public string? FecUltRenta { get; set; }

    public string? Producto { get; set; }

    public string? Marca { get; set; }

    public string? ValorResidual { get; set; }

    public string? Pago { get; set; }

    public string? Periodicidad { get; set; }

    public string? Balloon { get; set; }

    public string? Rfc { get; set; }

    public string? Dirección { get; set; }

    public string? Colonia { get; set; }

    public string? Cp { get; set; }

    public string? Ciudad { get; set; }

    public string? Estado { get; set; }

    public string? Tel1 { get; set; }

    public string? Tel2 { get; set; }

    public string? Tel3 { get; set; }

    public string? Llamoasc { get; set; }

    public string? Tel4 { get; set; }

    public string? Correo { get; set; }

    public string? F5 { get; set; }

    public string? F8 { get; set; }

    public string? Fecha1Renta { get; set; }

    public string? FechaUltimaRenta { get; set; }

    public string? F13 { get; set; }

    public string? Vehículo { get; set; }

    public string? F15 { get; set; }

    public string? Mensualidad { get; set; }

    public string? F17 { get; set; }

    public string? F18 { get; set; }

    public string? F19 { get; set; }

    public string? TelCasa { get; set; }

    public string? Celular { get; set; }

    public string? TelOtro { get; set; }

    public string? TelOficina { get; set; }

    public string? NoDeCliente { get; set; }

    public string? NombreDelCliente { get; set; }

    public string? RefAlfanum { get; set; }

    public string? VehiculoFinanciado { get; set; }

    public string? PlazoContratado { get; set; }

    public string? TelCelular { get; set; }

    public string? TelOficinaOtro { get; set; }

    public string? TelOtros2 { get; set; }

    public string? CalleNo { get; set; }

    public string? Poblacion { get; set; }

    public string? CodigoPostal { get; set; }

    public string? EntFed { get; set; }

    public string? Vin { get; set; }

    public string? Email { get; set; }

    public string? PersonaJuridica { get; set; }

    public string? FecInicio { get; set; }

    public string? RefCobro1 { get; set; }

    public string? Anualidad { get; set; }

    public string? EdoEquipo { get; set; }

    public string? TipoCrédito { get; set; }

    public string? Persona { get; set; }

    public string? Correro { get; set; }

    public string? TipoCredito { get; set; }

    public string? TelCelular1 { get; set; }

    public string? TelOtros21 { get; set; }

    public string? Vin2 { get; set; }

    public string? TipoOp { get; set; }

    public string? Domiciliado { get; set; }

    public string? Calle { get; set; }

    public string? Municipio { get; set; }

    public string? Edo { get; set; }

    public string? Dealer1 { get; set; }

    public string? TasaFinanciAda { get; set; }

    public string? TBase { get; set; }

    public string? FecFact { get; set; }

    public string? NoFact { get; set; }

    public string? Modelo { get; set; }

    public string? Comisiones { get; set; }

    public string? AV { get; set; }

    public string? PaqProm { get; set; }

    public string? TasaMoratoria { get; set; }

    public string? MontoOtrosFinanciados { get; set; }

    public string? TotalOtrosFinanciados { get; set; }

    public string? Sucursal { get; set; }

    public string? IdSolicitud { get; set; }

    public string? Pplan { get; set; }
}
