using System;
using System.Collections.Generic;

namespace HadesLibrary.ModelsDbHistory;

public partial class AcumProducto51
{
    public DateOnly? Insert { get; set; }

    public int IdLogAsignación { get; set; }

    public string? Despacho { get; set; }

    public string? FecAsig { get; set; }

    public string? Rfc { get; set; }

    public string IdCuenta { get; set; } = null!;

    public string? Solicitud { get; set; }

    public string? Origen { get; set; }

    public string? Portafolio { get; set; }

    public string? TCredito { get; set; }

    public string? MoracontI { get; set; }

    public string? SdocontI { get; set; }

    public string? MoraaplcI { get; set; }

    public string? MinimoIni { get; set; }

    public string? VencidIni { get; set; }

    public string? DiasmIni { get; set; }

    public string? Persona { get; set; }

    public string? FechaOrig { get; set; }

    public string? Plazo { get; set; }

    public string? Plaza { get; set; }

    public string? Regional { get; set; }

    public string? Division { get; set; }

    public string? NombreCte { get; set; }

    public string? Email { get; set; }

    public string? Cp { get; set; }

    public string? Calle { get; set; }

    public string? NExt { get; set; }

    public string? NIn { get; set; }

    public string? Colonia { get; set; }

    public string? Delegacion { get; set; }

    public string? Ciudad { get; set; }

    public string? Estado { get; set; }

    public string? Tel { get; set; }

    public string? Tel1 { get; set; }

    public string? NombreCon { get; set; }

    public string? TelConyug { get; set; }

    public string? TelOfic { get; set; }

    public string? MarcaAuto { get; set; }

    public string? SubMarca { get; set; }

    public string? Modelo { get; set; }

    public string? Serie { get; set; }

    public string? Motor { get; set; }

    public string? DiasmIni2 { get; set; }

    public string? Encargado { get; set; }

    public string? Estrategia { get; set; }

    public string? FechaArchivoPagos { get; set; }

    public string? MinimoIni2 { get; set; }

    public string? MoraaplcI2 { get; set; }

    public string? MoracontI2 { get; set; }

    public string? Prioridad { get; set; }

    public string? SdocontI2 { get; set; }

    public string? Solicitud2 { get; set; }

    public string? VencidIni2 { get; set; }

    public decimal? SumasdocontI { get; set; }

    public string? Sucursal { get; set; }

    public string? Periferia { get; set; }

    public string? Municipio { get; set; }

    public string? FechaActualizacion { get; set; }

    public string? Despacho1 { get; set; }

    public string? FecAsig1 { get; set; }

    public string? CartIniCuadre { get; set; }

    public string? FecAsigCuadre { get; set; }

    public string? MoraAplcCuadre { get; set; }

    public string? CartIni { get; set; }

    public string? Servicio { get; set; }

    public string? FechFactura { get; set; }

    public string? NoFactura { get; set; }

    public string? MoraCliente { get; set; }

    public string? Contratos { get; set; }

    public string? Derechos { get; set; }

    public string? Facturas { get; set; }

    public string? Intereses { get; set; }

    public string? Mantenimiento { get; set; }

    public string? Rentas { get; set; }

    public string? RentasAdicionales { get; set; }

    public string? Seguros { get; set; }

    public string? Derechos1 { get; set; }

    public string? EquipoCall { get; set; }

    public string? GranTotalFacturas { get; set; }

    public string? Intereses1 { get; set; }

    public string? Mantenimiento1 { get; set; }

    public string? Rentas1 { get; set; }

    public string? RentasAdicionales1 { get; set; }

    public string? Segmento { get; set; }

    public string? Seguros1 { get; set; }

    public string? Contratos1 { get; set; }

    public string? Derechos2 { get; set; }

    public string? Facturas1 { get; set; }

    public string? Intereses2 { get; set; }

    public string? Mantenimiento2 { get; set; }

    public string? Rentas2 { get; set; }

    public string? RentasAdicionales2 { get; set; }

    public string? Seguros2 { get; set; }

    public string? Cliente { get; set; }

    public string? GranTotal { get; set; }

    public string? IdSap { get; set; }

    public string? MontoFacturas { get; set; }

    public string? MontoInteres { get; set; }

    public string? Personas { get; set; }

    public string? Referencia { get; set; }

    public string? TotalFacturas { get; set; }

    public string? TotalIntereses { get; set; }

    public string? MAnexo { get; set; }

    public string? MoraAnexo { get; set; }

    public string? Origen1 { get; set; }
}
