using System;
using System.Collections.Generic;

namespace HadesLibrary.ModelsDbCollection;

public partial class GeneraCartera
{
    public decimal Idasignaciones { get; set; }

    public DateTime Fecharecepcion { get; set; }

    public string? NumeroDeCuenta { get; set; }

    public string? NombreDeudor { get; set; }

    public string? DirCalleNumero { get; set; }

    public string? DirColonia { get; set; }

    public string? DirDelMun { get; set; }

    public string? CodigoPostalPart { get; set; }

    public string? TelParticular { get; set; }

    public string? NombreEmpresa { get; set; }

    public string? Puesto { get; set; }

    public string? DirTrabajoCalleNumero { get; set; }

    public string? DirTrabajoColonia { get; set; }

    public string? DirTrabajoEstado { get; set; }

    public string? CodigoPostalTrab { get; set; }

    public string? TelTrabajo { get; set; }

    public string? Extension { get; set; }

    public string? ClaveCliente { get; set; }

    public string? Remesa { get; set; }

    public string? Referencia { get; set; }

    public string? NumeroDeEstado { get; set; }

    public string? ClaveDeZona { get; set; }

    public string? ClaveDeSucursal { get; set; }

    public string? TipoDeCredito { get; set; }

    public string? GrupoDeTrabajo { get; set; }

    public string? GrupoAnterior { get; set; }

    public string? FechaApertura { get; set; }

    public string? FechaCorte { get; set; }

    public string? FechaCancel { get; set; }

    public string? FechaCastigo { get; set; }

    public string? NumPagosVencidos { get; set; }

    public string? SaldoInicial { get; set; }

    public string? SaldoInicialDolares { get; set; }

    public string? SaldoVencido { get; set; }

    public string? SaldoVencidoDolares { get; set; }

    public string? InteresesFacturados { get; set; }

    public string? InteresesNoFacturados { get; set; }

    public string? SaldoActualPesos { get; set; }

    public string? SaldoActualDolares { get; set; }

    public string? SaldoAcobrar { get; set; }

    public string? SaldoAcobrarDolares { get; set; }

    public string? FechaAsigCliente { get; set; }

    public string? FechaAsigArchivo { get; set; }

    public string? FechaDeIngresoAcjc { get; set; }

    public string? FechaReasig { get; set; }

    public string? FechaActualizacion { get; set; }

    public string? FechaDevolucion { get; set; }

    public string? AcumuladoPagos { get; set; }

    public string? AcumuladoPagosDolares { get; set; }

    public string? Llave1 { get; set; }

    public string? Llave2 { get; set; }

    public string? Llave3 { get; set; }

    public string? FechaRevision { get; set; }

    public string? StatusCuenta { get; set; }

    public string? StatusEquivCliente { get; set; }

    public string? ClaveContacto { get; set; }

    public string? StatusUltimaGestTel { get; set; }

    public string? FechaUltimaGestTel { get; set; }

    public string? ClaveUltimoGestTel { get; set; }

    public string? FechaSegGestTel { get; set; }

    public string? HoraSegGestTel { get; set; }

    public string? LlamadasCcontacto { get; set; }

    public string? LlamadasScontacto { get; set; }

    public string? StatusUltimaGestDom { get; set; }

    public string? FechaUltimaGestDom { get; set; }

    public string? ClaveUltimoGestDom { get; set; }

    public string? FechaSegGestDom { get; set; }

    public string? HoraSeguimGestDom { get; set; }

    public string? VisitasCcontacto { get; set; }

    public string? VisitasScontacto { get; set; }

    public string? NumRegistrosDatosNuevos { get; set; }

    public string? NumeroAvales { get; set; }

    public string? Base { get; set; }

    public string? Nota { get; set; }

    public string? Val { get; set; }

    public string? MarcadoParaHistorico { get; set; }

    public string? FechaActOper { get; set; }

    public string? FechaActMov { get; set; }

    public string? Gestor { get; set; }

    public string? ClaveDeAsignacion { get; set; }

    public string? Estrategia { get; set; }

    public string? Estilo { get; set; }

    public string? TelAd1 { get; set; }

    public string? TelAd2 { get; set; }

    public string? TelAd3 { get; set; }

    public string? AdNum1 { get; set; }

    public string? AdNum2 { get; set; }

    public string? AdNum3 { get; set; }

    public string? AdNum4 { get; set; }

    public string? AdNum5 { get; set; }

    public string? AdNum6 { get; set; }

    public string? AdNum7 { get; set; }

    public string? AdNum8 { get; set; }

    public string? AdNum9 { get; set; }

    public string? AdNum0 { get; set; }

    public string? AdCar0 { get; set; }

    public string? AdCar1 { get; set; }

    public string? AdCar2 { get; set; }

    public string? AdCar3 { get; set; }

    public string? AdCar4 { get; set; }

    public string? AdCar5 { get; set; }

    public string? AdCar6 { get; set; }

    public string? AdCar7 { get; set; }

    public string? AdCar8 { get; set; }

    public string? AdCar9 { get; set; }

    public string? Log1 { get; set; }

    public string? Log2 { get; set; }

    public string? Log3 { get; set; }

    public string? SaldoInffinix { get; set; }

    public decimal? ActivasHoy { get; set; }

    public decimal? TipoCuentaCartera { get; set; }
}
