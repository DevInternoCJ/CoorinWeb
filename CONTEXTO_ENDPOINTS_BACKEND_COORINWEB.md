...
| `POST /api/reportes-cliente/generar` | JSON body | `GenerarReporteRequestDto`. `GET /api/reportes-cliente/definiciones` no recibe body ni parámetros. |
...
| `POST /api/reportes/ProductividadInfo/consultar` | JSON body | `ProductividadRequestDto`: `IdCartera`, `IdProducto` (nullable), `FechaInicial`, `FechaFinal`. |
